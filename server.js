...
async function readDb() {
  if (!pool) return readDbFile();
  const result = await pool.query("SELECT data FROM app_state WHERE id = 1");
  return normalizeDb(result.rows[0]?.data || emptyDb());
}

async function writeDb(db) {
  db = normalizeDb(db);
  if (!pool) return writeDbFile(db);
  await pool.query(
...
function mergeCommentFields(saved = {}, incoming = {}) {
  ...
  if (savedCurrent && incomingCurrent) {
    if (sameCommentAuthor(saved, incoming)) current = incomingCurrent;
    else { push(savedCurrent); current = incomingCurrent; }
  }
...
async function handleApi(req, res, pathname) {
  if (pathname === "/api/health" && req.method === "GET") {
    sendJson(res, 200, { ok: true, storage: pool ? "postgresql" : "db.json", websocket: true });
    return true;
  }
  ...
  if (pathname === "/api/state" && req.method === "PUT") {
    ...
    const db = mergeState(await readDb(), body || {});
    await writeDb(db);
...
async function start() {
  ...
  server.listen(port, "0.0.0.0", () => {
    ...
    console.log(`Storage: ${pool ? "PostgreSQL" : "db.json fallback"}; WebSocket: /ws`);
  });
}
