...
const SERVER_VERSION = "v868"; const REQUIRE_POSTGRES = ["1", "true", "on", "yes"].includes(String(process.env.REQUIRE_POSTGRES || "").trim().toLowerCase()); const LOCAL_STATE_MIRROR_ENABLED = ["1", "true", "on", "yes"].includes(String(process.env.PPR_LOCAL_STATE_MIRROR || (REQUIRE_POSTGRES ? "false" : "true")).trim().toLowerCase());
...
const SUPPORTED_CLIENT_VERSIONS = new Set([
  ...
  "v700-mobile-startup-rollback-1",
...
const PRESS_2400_PRODUCTION_NODES = Object.freeze([
...
function restoreGasQrCatalog(db) {
  ...
  const now = new Date().toISOString();
  const oldIndexByName = new Map(currentNames.map((name, index) => [name, index]));
  CATALOG_NODE_INDEXED_FIELDS.forEach(field => {
    const source = item[field];
    if (!source || typeof source !== "object" || Array.isArray(source)) return;
  ...
  (db.archivedNodeChecks || []).forEach((archive, archiveIndex) => {
    ...
    if (date && !nextChecks[restoredKey] && archive.record) nextChecks[restoredKey] = archive.record;
    restoredArchiveIndexes.add(archiveIndex);
  });
  db.checks = nextChecks;
  if (restoredArchiveIndexes.size) {
    db.archivedNodeChecks = db.archivedNodeChecks.filter((_, index) => !restoredArchiveIndexes.has(index));
  }
  const restoreLinkedNode = linked => {
    ...
    if (linked.archivedNodeName) {
      delete linked.archivedNode;
      delete linked.archivedNodeIndex;
      delete linked.archivedNodeName;
    }
  };
  ...
  item.gasQrRestoreVersion = "historical-order-v2";
  item.updatedAt = now;
  return true;
}
...
function archiveAndRemoveCraneBeamData(db) {
  ...
  Object.entries(db.catalog?.equipment || {}).forEach(([key, card]) => {
    ...
    const protectedPressNodes = String(card.id) === PRESS_2400_EQUIPMENT_ID
      ? new Set(PRESS_2400_PRODUCTION_NODES.map(normalizedCatalogNodeName))
      : new Set();
