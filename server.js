...
const SERVER_VERSION = "v533-rating-without-forklift-drivers";
...
async function handleApi(req, res, pathname, url) {
  ...
  if (pathname === "/api/admin/equipment/node-qr-rotate" && req.method === "POST") {
    if (req.authUser?.role !== "editor") { sendJson(res, 403, { ok: false, error: "admin_required" }); return true; }
    const body = await readBody(req).catch(() => ({}));
    const equipmentId = Number(body.equipmentId);
    const nodeIndex = Number(body.nodeIndex);
    if (!Number.isSafeInteger(equipmentId) || !Number.isSafeInteger(nodeIndex) || nodeIndex < 0) {
      sendJson(res, 400, { ok: false, error: "node_qr_rotate_invalid" }); return true;
    }
    const result = await enqueueStateWrite(async () => {
      const db = readDb();
      ...
      item.qrTokens[nodeIndex] = crypto.randomBytes(12).toString("hex");
      item.qrUpdatedAt = item.qrUpdatedAt && typeof item.qrUpdatedAt === "object" ? item.qrUpdatedAt : {};
      item.qrUpdatedAt[nodeIndex] = new Date().toISOString();
      item.updatedAt = item.qrUpdatedAt[nodeIndex];
      writeDb(db, { action: "equipment_node_qr_rotated", user: req.authUser, targetId: `${equipmentId}:${nodeIndex}`, targetLabel: item.nodes[nodeIndex] });
      return { item };
    });
  ...
  if (pathname === "/api/admin/maintenance" && req.method === "GET") {
    ...
    const systemReport = ["all", "report"].includes(requestedTab) ? systemReadinessReport(db, monitoringResult.snapshot, backups) : { status: "ok", summary: {}, checks: [], metrics: {}, environment: {}, policy: {} };
    const broadcasts = ["all", "broadcasts"].includes(requestedTab) ? (db.systemBroadcasts || []).slice().reverse().slice(0, 200).map(item => {
    ...
    const access = ["all", "access"].includes(requestedTab) ? (db.users || []).map(user => ({ ...userPublic(user), loginDiagnostics: userLoginDiagnostics(db, user), operationalSummary: adminUserOperationalSummary(db, user), instructionEditorCount: Object.values(db.workPermitInstructions || {}).filter(item => (item.editorIds || []).some(key => [user.id, user.employeeId, user.phone].map(String).includes(String(key)))).length })) : [];
    const integrity = ["all", "integrity", "report"].includes(requestedTab) ? dataIntegrityReport(db) : { healthy: true, fixableCount: 0, issues: [] };
    const activity = ["all", "activity"].includes(requestedTab) ? adminActivityFeed(db, req.authUser) : { readAt: "", unreadCount: 0, items: [] };
    sendJson(res, 200, { ok: true, trash, audit: ["all", "audit"].includes(requestedTab) ? (db.adminAuditLog || []).slice(0, requestedTab === "all" ? 1000 : 250) : [], access, broadcasts, instructionAcknowledgements: ["all", "instructionLog"].includes(requestedTab) ? (db.workPermitInstructionAcknowledgements || []).slice(0, 2000) : [], notificationPolicy: { defaultPriority: db.adminNotificationPolicy?.defaultPriority || "normal", defaultExpiryHours: Math.max(1, Math.min(720, Number(db.adminNotificationPolicy?.defaultExpiryHours || 24))), unreadReminderHours: Math.max(1, Math.min(168, Number(db.adminNotificationPolicy?.unreadReminderHours || 8))) }, activity, alerts: ["all", "monitoring"].includes(requestedTab) ? monitoringResult.alerts : [], monitoring: monitoringResult.snapshot, systemReport, automation: adminAutomationSnapshot(db), integrity, archivePreview: { days: archivePreview.days, cutoffAt: archivePreview.cutoffAt, counts: archivePreview.counts }, archives, postgres, backups, backupRetention, config: normalizedAdminConfig(db.adminConfig), configHistory: ["all", "settings"].includes(requestedTab) ? (db.adminConfigHistory || []).slice(0, 20).map(item => ({ id: item.id, at: item.at, actorName: item.actorName, reason: item.reason })) : [] });
    return true;
  }
