...
function broadcastState(origin = "server") {
  if (!wss) return;
  const message = JSON.stringify({ type: "state", origin, state: publicState() });
...
async function handleApi(req, res, pathname) {
  ...
  if (pathname === "/api/state" && req.method === "PUT") {
    ...
    broadcastState(body.clientId || "api");
    return true;
  }
  ...
  if (pathname === "/api/users" && req.method === "POST") {
    ...
    broadcastState(user.clientId || "user");
    return true;
  }
