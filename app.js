...
function commentEntryKey(entry) {
  return [entry?.text || "", entry?.photo || "", entry?.role || "", entry?.name || ""].join("|#|");
}

function mergeCommentItem(localItem = {}, remoteItem = {}) {
  const result = { ...(localItem || {}), ...(remoteItem || {}) };
  const log = [];
  const push = entry => {
    if (!entry || !String(entry.text || "").trim()) return;
    if (!log.some(item => commentEntryKey(item) === commentEntryKey(entry))) log.push(entry);
  };

  (Array.isArray(localItem.commentLog) ? localItem.commentLog : []).forEach(push);
  (Array.isArray(remoteItem.commentLog) ? remoteItem.commentLog : []).forEach(push);

  const localCurrent = currentCommentEntry(localItem);
  const remoteCurrent = currentCommentEntry(remoteItem);
  if (localCurrent && remoteCurrent && commentEntryKey(localCurrent) !== commentEntryKey(remoteCurrent)) {
    push(localCurrent);
    result.comment = remoteCurrent.text;
    result.commentPhoto = remoteCurrent.photo || "";
    result.commentOwnerRole = remoteCurrent.role || "";
    result.commentOwnerName = remoteCurrent.name || "";
    result.commentUpdatedAt = remoteCurrent.at || result.commentUpdatedAt || "";
  } else {
    const current = remoteCurrent || localCurrent;
    result.comment = current?.text || "";
    result.commentPhoto = current?.photo || "";
    result.commentOwnerRole = current?.role || "";
    result.commentOwnerName = current?.name || "";
    result.commentUpdatedAt = current?.at || result.commentUpdatedAt || "";
  }
  result.commentLog = log;
  return result;
}

function mergeCheckRecords(localChecks = {}, remoteChecks = {}) {
  const result = { ...(localChecks || {}) };
  Object.entries(remoteChecks || {}).forEach(([checkKey, remoteRecord]) => {
    const localRecord = result[checkKey] || {};
    const mergedRecord = { ...localRecord, ...(remoteRecord || {}) };
    ["to"].forEach(kind => {
      if (localRecord?.[kind] || remoteRecord?.[kind]) {
        mergedRecord[kind] = mergeCommentItem(localRecord?.[kind] || {}, remoteRecord?.[kind] || {});
      }
    });
...
function mergeRequestComments(localReq = {}, remoteReq = {}) {
  const result = { ...(localReq || {}), ...(remoteReq || {}) };
  const lines = [];
  const pushLines = value => String(value || "").split("\n").forEach(line => {
    const clean = line.trim();
    if (clean && !lines.includes(clean)) lines.push(clean);
  });
  pushLines(localReq.comment);
  pushLines(remoteReq.comment);
  result.comment = lines.join("\n");
  result.commentPhoto = remoteReq.commentPhoto || localReq.commentPhoto || "";
  result.requestPhoto = remoteReq.requestPhoto || localReq.requestPhoto || "";
  return result;
}

function mergeRequests(localRequests = {}, remoteRequests = {}) {
  const result = { ...(localRequests || {}) };
  Object.entries(remoteRequests || {}).forEach(([id, remoteReq]) => {
    result[id] = mergeRequestComments(result[id] || {}, remoteReq || {});
  });
  return result;
}
...
function scheduleRemoteReload(delay = 120) {
  clearTimeout(remoteReloadTimer);
  remoteReloadTimer = setTimeout(() => {
    if (!document.hidden) loadRemoteState();
  }, delay);
}
...
function connectRealtimeSync() {
  ...
  ws.addEventListener("message", event => {
    try {
      ...
      if (message.type === "state-updated") scheduleRemoteReload();
      if (message.type === "users-updated") loadRemoteUsers();
    } catch {}
  });

  ws.addEventListener("close", () => {
    clearTimeout(wsReconnectTimer);
    wsReconnectTimer = setTimeout(connectRealtimeSync, 3000);
  });
...
async function loadRemoteState() {
  try {
    ...
    state.checks = mergeCheckRecords(state.checks || {}, remote.checks || {});
    state.requests = mergeRequests(state.requests || {}, remote.requests || {});
...
function queueRemoteStateSave() {
  ...
  remoteSaveTimer = setTimeout(saveRemoteState, 100);
}
...
function visibleCommentEntries(item, includeEditable = true) {
  ...
  return (current ? [...log, current] : [...log]).filter(Boolean);
}
...
function renderChecklist() {
  ...
  ui.commentInput.value = commentInputValue(kind);
