...
const requestSubmitLocks = new Set();
const ui = {
...
function createNodeWalkRequestSubmission(equipmentId, nodeIndex, date, item, text, requestPhoto = "") {
  ...
  // Одна активная заявка на один узел/дату/ТО. Это убирает дубли от двойного клика
  // и от повторного вызова обработчика: повторная отправка обновляет ту же заявку, а не создает вторую.
  ...
  state.requests[id] = {
    ...
    area: eq?.area || "",
    node: eq?.nodes[nodeIndex] || "",
    comment: commentsSummary(item) || item.comment || old.comment || "",
    commentPhoto: item.commentPhoto || old.commentPhoto || "",
    text: cleanText,
    requestPhoto: requestPhoto || item.requestPhoto || old.requestPhoto || "",
    invoicePhoto: old.invoicePhoto || "",
    noInvoiceApproved: Boolean(old.noInvoiceApproved),
    price: old.price || "",
    supplier: old.supplier || "",
    status: old.status || "shop",
    shopApproved: Boolean(old.shopApproved),
    engineerApproved: Boolean(old.engineerApproved),
    supplyPrepared: Boolean(old.supplyPrepared),
    financeApproved: Boolean(old.financeApproved),
    cashApproved: Boolean(old.cashApproved),
    transferredToWarehouse: Boolean(old.transferredToWarehouse),
    warehouseReceived: Boolean(old.warehouseReceived),
    issued: Boolean(old.issued),
    mechanicInstalled: Boolean(old.mechanicInstalled),
    shopInstallApproved: Boolean(old.shopInstallApproved),
    accountingWrittenOff: Boolean(old.accountingWrittenOff),
    done: Boolean(old.done),
    stock: Boolean(old.stock),
    qtyReceived: Number(old.qtyReceived || 0),
    qtyIssued: Number(old.qtyIssued || 0),
    stockArea: old.stockArea || "",
    inventoryAddedQty: Number(old.inventoryAddedQty || 0),
...
function renderNodeWalkthrough(eq) {
  ...
  eq.nodes.forEach((nodeName, index) => {
    ...
    row.querySelector("[data-node-stop]")?.addEventListener("click", async event => {
      if (!canEditChecklist()) return;
      setButtonBusy(event.currentTarget, true, activeStop ? "Публикуется..." : "Выбор...");
    ...
    row.querySelector("[data-node-submit-request]").addEventListener("click", async event => {
      ...
      const submitKey = requestId(eq.id, index, current.date, "to");
      if (requestSubmitLocks.has(submitKey) || button.disabled || button.dataset.busy === "1") return;
      requestSubmitLocks.add(submitKey);
      button.dataset.busy = "1";
      item.request = row.querySelector("[data-node-request]").value;
      if (!item.request.trim()) {
        requestSubmitLocks.delete(submitKey);
        delete button.dataset.busy;
    ...
    row.querySelector("[data-node-fixed]").addEventListener("click", event => runButtonOperation(event.currentTarget, () => {
      ...
      if (profile?.role === "shop" || profile?.role === "engineer" || profile?.role === "editor") {
        markCommentResolved(item);
        item.mechanicFixed = false;
        // Закрытие комментария/замечания не должно менять маршрут заявки и отправлять ее в бухгалтерию.
        // В бухгалтерию заявка уходит только после отдельного подтверждения установки детали.
        if (item.request.trim()) {
          const req = upsertNodeWalkRequest(eq.id, index, current.date, item);
          syncRequestToRecord(req);
        } else {
        ...
        row.querySelector(".node-walk-status").textContent = item.request.trim() ? "Замечание закрыто. Маршрут заявки не изменен" : "Замечание закрыто";
        renderNodeWalkthrough(equipmentById(eq.id));
        return;
      }
