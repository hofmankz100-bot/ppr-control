...
function createNodeWalkRequestSubmission(equipmentId, nodeIndex, date, item, text, requestPhoto = "") {
  ...
  const recentDuplicate = Object.values(state.requests || {}).find(req =>
    Number(req.equipmentId) === Number(equipmentId)
    && Number(req.nodeIndex) === Number(nodeIndex)
    && req.date === date
    && req.kind === "to"
    && String(req.text || "").trim() === cleanText
    && String(req.requestPhoto || "") === String(requestPhoto || item.requestPhoto || "")
    && (Date.now() - new Date(req.createdAt || req.updatedAt || 0).getTime()) < 15000
  );
  if (recentDuplicate) {
    item.lastRequestId = recentDuplicate.id;
    item.request = "";
    item.requestPhoto = "";
...
function isRealIssuedInstallationRequest(req) {
  return Boolean(req && req.issued && !req.stock);
}

function clearLocalInstallFlags(item) {
  if (!item) return;
  item.mechanicFixed = false;
  item.mechanicInstalled = false;
...
function requestMatchesRole(req, role) {
  ...
  if (role === "accounting") return isRealIssuedInstallationRequest(req) && req.shopInstallApproved && !req.accountingWrittenOff;
  return false;
}
...
function waitingRole(req) {
  ...
  if (isRealIssuedInstallationRequest(req) && req.shopInstallApproved) return "done";
  if (isRealIssuedInstallationRequest(req) && req.mechanicInstalled && !req.shopInstallApproved && !req.done) return "confirmInstall";
...
function statusText(status) {
  ...
  return {
    ...
    accounting: "Установка подтверждена",
    done: "Выполнено"
  }[status] || status;
}
...
function renderNodeWalkthrough(eq) {
  ...
  eq.nodes.forEach((nodeName, index) => {
    ...
    row.querySelector("[data-node-fixed]").addEventListener("click", event => runButtonOperation(event.currentTarget, () => {
      ...
      // ВАЖНО: кнопка "Устранено" в обходе закрывает только замечание.
      // Она не должна двигать заявку по маршруту и не должна менять статус установки.
      markCommentResolved(item);
      clearLocalInstallFlags(item);
      item.commentPhoto = "";
      saveState();
      row.querySelector("[data-node-comment]").value = "";
...
function nodeWalkStatusText(item) {
  if (item.request?.trim() && item.issued && item.shopInstallApproved && !item.accountingWrittenOff && !item.done) return "Установка подтверждена. Ждет бухгалтерию";
...
function requestCard(req) {
  ...
  if (current.requestRole === "accounting" && canAct && isRealIssuedInstallationRequest(req) && req.shopInstallApproved && !req.accountingWrittenOff && !req.done) {
