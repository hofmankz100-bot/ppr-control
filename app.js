...
const APP_VERSION = "v163-compressor-journal-logic-fix";
...
const COMPRESSOR_JOURNAL_MAX_DATE = "2028-12-31";
...
function compressorJournalMonthStartISO(year = current.year, month = current.month) {
  return isoFromDate(new Date(year, month, 1));
}

function compressorJournalMonthEndISO(year = current.year, month = current.month) {
  return isoFromDate(new Date(year, month + 1, 0));
}

function compressorJournalClampMonth() {
  const min = new Date(`${todayISO()}T00:00:00`);
  const max = new Date(`${COMPRESSOR_JOURNAL_MAX_DATE}T00:00:00`);
  const currentMonth = new Date(current.year, current.month, 1);
  const minMonth = new Date(min.getFullYear(), min.getMonth(), 1);
  const maxMonth = new Date(max.getFullYear(), max.getMonth(), 1);
  if (currentMonth < minMonth) {
    current.year = minMonth.getFullYear();
    current.month = minMonth.getMonth();
  }
  if (currentMonth > maxMonth) {
    current.year = maxMonth.getFullYear();
    current.month = maxMonth.getMonth();
  }
}

function compressorJournalRows(area = COMPRESSOR_JOURNAL_AREA, year = current.year, month = current.month) {
  state.compressorJournal ||= {};
  const minDate = todayISO();
  const monthStart = compressorJournalMonthStartISO(year, month);
  const monthEnd = compressorJournalMonthEndISO(year, month);
  let date = monthStart < minDate ? minDate : monthStart;
  const lastDate = monthEnd > COMPRESSOR_JOURNAL_MAX_DATE ? COMPRESSOR_JOURNAL_MAX_DATE : monthEnd;
  const rows = [];
  while (date <= lastDate) {
...
function compressorJournalMonthComplete(area = COMPRESSOR_JOURNAL_AREA, year = current.year, month = current.month) {
  const rows = compressorJournalRows(area, year, month);
...
function gasSheetActionsHtml(section, complete) {
  ...
  return `<div class="gas-sheet-actions">
    ...
    ${complete ? `<button type="button" data-gas-print="${section}">🖨 Печать листа ${label} №${sheetNo}</button><button type="button" class="danger" data-gas-clear="${section}">🗑 Удалить лист ${label} №${sheetNo}</button>` : `<span class="gas-sheet-note">Печать и удаление появятся после заполнения хотя бы одной даты на этом листе.</span>`}
  </div>`;
}
...
function renderCompressorJournal(area = COMPRESSOR_JOURNAL_AREA) {
  ...
  const unlockedSheetCount = firstIncompleteSheet >= 0 ? firstIncompleteSheet + 1 : sheets.length;
  const visibleSheets = sheets.slice(0, unlockedSheetCount || 1);
  if (!Number.isInteger(current.compressorSheetIndex) || current.compressorSheetIndex < 0 || current.compressorSheetIndex >= visibleSheets.length) {
    current.compressorSheetIndex = Math.max(0, visibleSheets.length - 1);
  }
  const currentSheetRows = visibleSheets[current.compressorSheetIndex] || [];
  ...
  const todayComplete = compressorJournalDateComplete(area, todayISO());
  const minMonth = new Date(`${todayISO()}T00:00:00`);
  const maxMonth = new Date(`${COMPRESSOR_JOURNAL_MAX_DATE}T00:00:00`);
  const isMinMonth = current.year === minMonth.getFullYear() && current.month === minMonth.getMonth();
  const isMaxMonth = current.year === maxMonth.getFullYear() && current.month === maxMonth.getMonth();
  const monthComplete = compressorJournalMonthComplete(area, current.year, current.month);
  ui.aggregateJournalTitle.textContent = `\u0410\u0433\u0440\u0435\u0433\u0430\u0442\u043d\u044b\u0439 \u0436\u0443\u0440\u043d\u0430\u043b: ${area}`;
  ui.aggregateJournalMeta.textContent = `${filled} \u0437\u0430\u043f\u043e\u043b\u043d\u0435\u043d\u043d\u044b\u0445 \u0441\u0442\u0440\u043e\u043a. \u0421\u0435\u0433\u043e\u0434\u043d\u044f: ${todayComplete ? "\u0417\u0430\u043f\u043e\u043b\u043d\u0435\u043d\u043e" : "\u041d\u0435 \u0437\u0430\u043f\u043e\u043b\u043d\u0435\u043d\u043e"}. \u0414\u0430\u0442\u044b \u043e\u0442 \u0441\u0435\u0433\u043e\u0434\u043d\u044f \u0434\u043e 31.12.2028.`;
  ui.aggregateJournalList.innerHTML = `
    <div class="aggregate-print-actions compressor-journal-actions">
      <div class="segmented">
        <button type="button" data-compressor-month="prev" ${isMinMonth ? "disabled" : ""}>\u2039</button>
        <strong>${new Date(current.year, current.month, 1).toLocaleDateString("ru-RU", { month: "long", year: "numeric" })}</strong>
        <button type="button" data-compressor-month="next" ${(isMaxMonth || !monthComplete) ? "disabled" : ""}>\u203a</button>
      </div>
      ${todayComplete ? `<span class="status-badge done">\u0417\u0430\u043f\u043e\u043b\u043d\u0435\u043d\u043e</span>` : `<span class="status-badge danger">\u041d\u0435 \u0437\u0430\u043f\u043e\u043b\u043d\u0435\u043d\u043e</span>`}
      ${currentSheetComplete && current.compressorSheetIndex < visibleSheets.length - 1 ? `<button type="button" data-compressor-next-sheet>\u041e\u0442\u043a\u0440\u044b\u0442\u044c \u0441\u043b\u0435\u0434\u0443\u044e\u0449\u0438\u0439 \u043b\u0438\u0441\u0442</button>` : ""}
    </div>
    ${visibleSheets.map((sheetRows, sheetIndex) => `
    <div class="aggregate-journal-sheet compressor-journal-sheet ${sheetIndex === current.compressorSheetIndex ? "" : "compressor-sheet-hidden"}" data-compressor-sheet="${sheetIndex}">
      <div class="aggregate-sheet-head">
        ...
        <span>${compressorJournalSheetComplete(sheetRows) ? "\u0417\u0430\u043f\u043e\u043b\u043d\u0435\u043d\u043e" : "\u041d\u0435 \u0437\u0430\u043f\u043e\u043b\u043d\u0435\u043d\u043e"}</span>
        <button class="compressor-sheet-print" type="button" data-print-compressor-sheet="${sheetIndex}">\u041f\u0435\u0447\u0430\u0442\u044c \u043b\u0438\u0441\u0442\u0430 \u2116${sheetIndex + 1}</button>
      </div>
  ...
  ui.aggregateJournalList.querySelector("[data-compressor-month='prev']")?.addEventListener("click", event => {
  ...
  ui.aggregateJournalList.querySelector("[data-compressor-month='next']")?.addEventListener("click", event => {
    if (event.currentTarget.disabled || !compressorJournalMonthComplete(area, current.year, current.month)) return;
    current.month += 1;
    if (current.month > 11) {
      current.month = 0;
      current.year += 1;
