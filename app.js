...
function startRealtimeSync() {
  ...
  realtimeSocket.addEventListener("message", event => {
    try {
      ...
      if (msg.type === "state" && msg.origin !== CLIENT_ID) mergeRemoteState(msg.state);
    } catch {}
  });
...
async function saveRemoteState() {
  ...
  try {
    await apiJson("/api/state", {
      method: "PUT",
      body: JSON.stringify({ clientId: CLIENT_ID, checks: state.checks, requests: state.requests, inventory: state.inventory, catalog: state.catalog, directorMessages: state.directorMessages, downtimes: state.downtimes || [] })
    });
  } catch {
