...
function loadState() {
  try {
    ...
    return { checks: {}, requests: {}, inventory: {}, catalog: { equipment: {} }, directorMessages: [], downtimes: [], compressorJournal: {} };
  }
}
    ...
    .forEach(eq => {
      ...
      tr.innerHTML = `
        ...
        <td class="equipment-journal-cell area-color-cell"${downtimeStyle}>
          <button type="button" data-aggregate-area="${escapeHtml(eq.area)}" class="${compressorJournalMissingToday ? "compressor-journal-alert" : ""}">
            <span>Агрегатный журнал</span>
            <strong>${escapeHtml(eq.area)}</strong>
            <small>${eq.area === COMPRESSOR_JOURNAL_AREA ? (compressorJournalMissingToday ? "сегодня не заполнено" : "сегодня заполнено") : `${aggregateJournalCount(eq.area)} записей`}</small>
          </button>
        </td>
      `;
