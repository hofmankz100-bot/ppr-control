...
const APP_VERSION = "v94-realtime";
...
async function publishStateNow(actionId = "") {
  clearTimeout(remoteSaveTimer);
  await saveRemoteState(actionId);
}

function waitForRealtimeAction(actionId, timeoutMs = 6000) {
  if (!actionId || !realtimeSocket || realtimeSocket.readyState !== WebSocket.OPEN) return Promise.resolve(false);
...
function confirmRealtimeAction(actionId) {
  const done = pendingActions.get(actionId);
  if (done) done();
}
...
async function runButtonOperation(button, handler, text = "Ожидание...") {
  if (!button || button.disabled) return;
  const actionId = createActionId();
  setButtonBusy(button, true, text);
  try {
    await Promise.resolve(handler());
    if (button.isConnected) button.textContent = "Синхронизация...";
    const realtimeAck = waitForRealtimeAction(actionId);
    await publishStateNow(actionId);
    await realtimeAck;
...
function mergeRemoteState(remote, shouldRender = true) {
  if (!remote || typeof remote !== "object") return;
  state.checks = remote.checks || {};
  state.requests = remote.requests || {};
  state.inventory = remote.inventory || {};
  state.catalog = remote.catalog || { equipment: {} };
  state.catalog.equipment ||= {};
  state.directorMessages = remote.directorMessages || [];
  state.downtimes = remote.downtimes || [];
  localStorage.setItem(STORE_KEY, JSON.stringify(state));
...
async function loadRemoteState() {
  try {
    mergeRemoteState(await apiJson("/api/state"));
  } catch {
...
function startRealtimeSync() {
  ...
  realtimeSocket.addEventListener("message", event => {
    try {
      ...
      if (msg.type === "state") {
        if (msg.actionId) confirmRealtimeAction(msg.actionId);
        if (msg.origin !== CLIENT_ID) mergeRemoteState(msg.state);
      }
    } catch {}
  });
  realtimeSocket.addEventListener("close", () => {
    realtimeReconnectTimer = setTimeout(startRealtimeSync, 1500);
  });
...
async function saveRemoteState(actionId = "") {
  if (remoteSaveInFlight) {
    remoteSavePending = true;
    await remoteSavePromise;
    return actionId ? saveRemoteState(actionId) : remoteSavePromise;
  }
  ...
  try {
    await apiJson("/api/state", {
      method: "PUT",
      body: JSON.stringify({ clientId: CLIENT_ID, actionId, checks: state.checks, requests: state.requests, inventory: state.inventory, catalog: state.catalog, directorMessages: state.directorMessages, downtimes: state.downtimes || [] })
    });
  } catch {
...
function renderNodeWalkthrough(eq) {
  ...
  eq.nodes.forEach((nodeName, index) => {
    ...
    row.querySelector("[data-node-stop]")?.addEventListener("click", async event => {
      if (!canEditChecklist()) return;
      await runButtonOperation(event.currentTarget, () => toggleDowntime(eq.id, index), activeStop ? "Ожидание..." : "Выбор...");
    });
    ...
    row.querySelector("[data-node-submit-comment]").addEventListener("click", async event => {
      ...
      await runButtonOperation(event.currentTarget, async () => {}, "Ожидание...");
      renderNodeWalkthrough(equipmentById(eq.id));
    });
    ...
    row.querySelector("[data-node-submit-request]").addEventListener("click", async event => {
      ...
      await runButtonOperation(button, async () => {}, "Ожидание...");
      renderNodeWalkthrough(equipmentById(eq.id));
    });
