...
const APP_VERSION = "v163-compressor-checker-column";
...
function gasJournalKey(area, section, date) {
  return `${area}::${section}::${date}`;
}
...
function gasJournalRowComplete(row, section) {
  const fields = section === "A"
    ? ["time", "inPressure", "outPressure", "temperature", "filterDelta", "regulator", "pzk", "psk", "result", "signedBy"]
    : ["time", "controlPipes", "gasSmell", "securityZone", "measures", "signedBy"];
...
function gasJournalTodayNeedsAttention(area) {
  ...
  const date = todayISO();
  const a = { ...(state.gasJournal?.[gasJournalKey(area, "A", date)] || {}) };
  const b = { ...(state.gasJournal?.[gasJournalKey(area, "B", date)] || {}) };
  return !gasJournalRowComplete(a, "A") || !gasJournalRowComplete(b, "B");
}

function gasJournalFilledCount(area) {
  return Object.values(state.gasJournal || {})
    .filter(row => row?.area === area)
    .filter(row => ["time", "inPressure", "outPressure", "temperature", "filterDelta", "regulator", "pzk", "psk", "result", "controlPipes", "gasSmell", "securityZone", "remarks", "measures", "signedBy"].some(field => String(row[field] || "").trim()))
    .length;
}
...
function updateGasJournalRow(rowId, field, value) {
  ...
  const next = {
    ...
    section,
    date,
    time: old.time || localTimeNow(),
    signedBy: old.signedBy || profile?.name || "",
    [field]: value,
...
function deleteGasJournalSheet(area, section, sheetIndex) {
  ...
  const start = Number(sheetIndex) * GAS_JOURNAL_DAYS_PER_SHEET;
  rows.slice(start, start + GAS_JOURNAL_DAYS_PER_SHEET).forEach(row => delete state.gasJournal[row.id]);
  saveState();
...
function gasChoiceSelect(row, field, values) {
  ...
  return `<select data-gas-row="${escapeHtml(row.id)}" data-gas-field="${field}">
    <option value=""></option>
...
function renderGasJournal(area) {
  ...
  ui.aggregateJournalList.innerHTML = `
    <div class="aggregate-print-actions gas-journal-actions">
      <div class="segmented">
        <button type="button" data-gas-month="prev">‹</button>
        <strong>${monthName}</strong>
        <button type="button" data-gas-month="next">›</button>
      </div>
      <div class="segmented">
        <button type="button" class="${section === "A" ? "active" : ""}" data-gas-section="A">Раздел А — ГРП/ГРУ</button>
        <button type="button" class="${section === "B" ? "active" : ""}" data-gas-section="B">Раздел Б — подземный газопровод</button>
      </div>
    </div>
    ${missingToday ? `<div class="empty-state request-alert">Кнопка журнала мигает, потому что за текущую дату нет полной записи в разделе А или Б.</div>` : ""}
    ${sheets.map((sheetRows, sheetIndex) => `
      <div class="aggregate-journal-sheet gas-journal-sheet ${sheetIndex === current.gasSheetIndex ? "" : "compressor-sheet-hidden"}" data-gas-sheet="${sheetIndex}">
        <div class="aggregate-sheet-head">
          <strong>${section === "A" ? "Раздел А. Эксплуатация и ТО ГРП (ГРУ)" : "Раздел Б. Обход подземного газопровода"}</strong>
          <span>${monthName} · Лист №${sheetIndex + 1} раздела ${section}</span>
          <div class="gas-sheet-buttons">
            <button type="button" data-print-gas-sheet="${sheetIndex}">Печатать</button>
            <button type="button" class="danger" data-delete-gas-sheet="${sheetIndex}">Удалить лист</button>
          </div>
        </div>
        <div class="aggregate-journal-table-wrap">
          ${section === "A" ? renderGasJournalSectionATable(sheetRows) : renderGasJournalSectionBTable(sheetRows)}
        </div>
      </div>
    `).join("")}
  `;
  ui.aggregateJournalList.querySelectorAll("[data-gas-section]").forEach(button => button.addEventListener("click", () => {
    current.gasSection = button.dataset.gasSection;
    current.gasSheetIndex = 0;
    renderAggregateJournal();
  }));
  ui.aggregateJournalList.querySelector("[data-gas-month='prev']")?.addEventListener("click", () => {
    current.month -= 1;
    if (current.month < 0) { current.month = 11; current.year -= 1; }
    current.gasSheetIndex = 0;
    renderAggregateJournal();
  });
  ui.aggregateJournalList.querySelector("[data-gas-month='next']")?.addEventListener("click", () => {
    current.month += 1;
    if (current.month > 11) { current.month = 0; current.year += 1; }
    current.gasSheetIndex = 0;
    renderAggregateJournal();
  });
  ui.aggregateJournalList.querySelectorAll("[data-print-gas-sheet]").forEach(button => button.addEventListener("click", () => {
    const sheetIndex = button.dataset.printGasSheet || "0";
    const sheet = ui.aggregateJournalList.querySelector(`[data-gas-sheet="${CSS.escape(sheetIndex)}"]`);
    if (!sheet) return;
    const cleanup = () => { document.body.classList.remove("print-compressor-single"); sheet.classList.remove("print-selected"); window.removeEventListener("afterprint", cleanup); };
  ...
  ui.aggregateJournalList.querySelectorAll("[data-delete-gas-sheet]").forEach(button => button.addEventListener("click", () => {
    if (confirm("Удалить заполнение этого листа и открыть новый пустой лист?")) deleteGasJournalSheet(area, section, button.dataset.deleteGasSheet);
  }));
  ui.aggregateJournalList.querySelectorAll("[data-gas-row]").forEach(control => {
    const commit = event => {
      const el = event.currentTarget;
      const saved = updateGasJournalRow(el.dataset.gasRow, el.dataset.gasField, el.value.trim());
      const timeCell = ui.aggregateJournalList.querySelector(`[data-gas-time="${CSS.escape(el.dataset.gasRow)}"]`);
      const signCell = ui.aggregateJournalList.querySelector(`[data-gas-sign="${CSS.escape(el.dataset.gasRow)}"]`);
      if (timeCell) timeCell.textContent = saved.time || "";
      if (signCell) signCell.textContent = saved.signedBy || "";
    };
    control.addEventListener("change", event => { commit(event); renderAggregateJournal(); });
    control.addEventListener("blur", commit);
  });
}

function renderGasJournalSectionATable(rows) {
  return `<table class="aggregate-journal-table gas-journal-table">
    <thead><tr>
      <th>Дата</th><th>Время</th><th>Вход МПа</th><th>Выход МПа</th><th>Темп. °C</th><th>ΔP фильтра</th><th>Регулятор</th><th>ПЗК</th><th>ПСК</th><th>Результаты ТО</th><th>Подпись</th>
    </tr></thead>
    <tbody>${rows.map(row => `<tr class="${gasJournalRowComplete(row, "A") ? "" : "open"}">
      <td>${dateHuman(row.date)}</td>
      <td data-gas-time="${escapeHtml(row.id)}">${escapeHtml(row.time || "")}</td>
      <td><input data-gas-row="${escapeHtml(row.id)}" data-gas-field="inPressure" value="${escapeHtml(row.inPressure || "")}" inputmode="decimal"></td>
      <td><input data-gas-row="${escapeHtml(row.id)}" data-gas-field="outPressure" value="${escapeHtml(row.outPressure || "")}" inputmode="decimal"></td>
      <td><input data-gas-row="${escapeHtml(row.id)}" data-gas-field="temperature" value="${escapeHtml(row.temperature || "")}" inputmode="decimal"></td>
      <td><input data-gas-row="${escapeHtml(row.id)}" data-gas-field="filterDelta" value="${escapeHtml(row.filterDelta || "")}" inputmode="decimal"></td>
      <td>${gasChoiceSelect(row, "regulator", ["Исправно", "Неисправно"])}</td>
      <td>${gasChoiceSelect(row, "pzk", ["Исправно", "Неисправно"])}</td>
      <td>${gasChoiceSelect(row, "psk", ["Исправно", "Неисправно"])}</td>
      <td><input data-gas-row="${escapeHtml(row.id)}" data-gas-field="result" value="${escapeHtml(row.result || "")}" placeholder="Замечаний нет"></td>
      <td data-gas-sign="${escapeHtml(row.id)}">${escapeHtml(row.signedBy || "")}</td>
    </tr>`).join("")}</tbody>
  </table>`;
}
...
function renderGasJournalSectionBTable(rows) {
  ...
  return `<table class="aggregate-journal-table gas-journal-table">
    <thead><tr>
      <th>Дата</th><th>Время</th><th>Участок</th><th>Контрольные трубки и колодцы</th><th>Запах газа</th><th>Охранная зона</th><th>Замечания</th><th>Принятые меры</th><th>Подпись</th>
    </tr></thead>
    <tbody>${rows.map(row => `<tr class="${gasJournalRowComplete(row, "B") ? "" : "open"}">
      <td>${dateHuman(row.date)}</td>
      <td data-gas-time="${escapeHtml(row.id)}">${escapeHtml(row.time || "")}</td>
      <td class="gas-route-cell">${escapeHtml(routeText)}</td>
      <td>${gasChoiceSelect(row, "controlPipes", ["Исправны", "Неисправны"])}</td>
      <td>${gasChoiceSelect(row, "gasSmell", ["Нет", "Есть"])}</td>
      <td>${gasChoiceSelect(row, "securityZone", ["Без нарушений", "Нарушение"])}</td>
      <td><input data-gas-row="${escapeHtml(row.id)}" data-gas-field="remarks" value="${escapeHtml(row.remarks || "")}" placeholder="Нет"></td>
      <td>${gasChoiceSelect(row, "measures", ["Не требуется", "Требуется"])}</td>
      <td data-gas-sign="${escapeHtml(row.id)}">${escapeHtml(row.signedBy || "")}</td>
    </tr>`).join("")}</tbody>
  </table>`;
}
...
function renderEquipment() {
  ...
  equipment
    ...
    .forEach(eq => {
      ...
      const journalMissingToday = compressorJournalMissingToday || gasJournalMissingToday;
      tr.innerHTML = `
        ...
        <td class="equipment-journal-cell area-color-cell"${downtimeStyle}>
          <button type="button" data-aggregate-area="${escapeHtml(eq.area)}" class="${journalMissingToday ? "compressor-journal-alert" : ""}">
            <span>Агрегатный журнал</span>
            <strong>${escapeHtml(eq.area)}</strong>
            <small>${eq.area === COMPRESSOR_JOURNAL_AREA || isGasJournalArea(eq.area) ? (journalMissingToday ? "сегодня не заполнено" : "сегодня заполнено") : `${aggregateJournalCount(eq.area)} записей`}</small>
          </button>
        </td>
      `;
