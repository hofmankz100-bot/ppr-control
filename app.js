...
const APP_VERSION = "v163-gas-journal-multipledays-sheet";
...
const GAS_JOURNAL_DAYS_PER_SHEET_A = 12;
const GAS_JOURNAL_DAYS_PER_SHEET_B = 3;
...
function gasSheetActionsHtml(section, complete) {
  ...
  return `<div class="gas-sheet-actions">
    ...
    ${complete ? `<button type="button" data-gas-print="${section}">🖨 Печать листа ${label} №${sheetNo}</button><button type="button" class="danger" data-gas-clear="${section}">🗑 Удалить лист ${label} №${sheetNo}</button>` : `<span class="gas-sheet-note">Печать и удаление появятся после полного заполнения всех дат на этом листе.</span>`}
  </div>`;
}
...
function renderGasJournal() {
  ...
  const completeA = gasJournalSheetComplete("A");
  const completeB = gasJournalSheetComplete("B");
          ...
          <tbody>
            ${datesA.map(date => {
              ...
              return `<tr class="gas-day-row">
                ...
                <td>${gasInputHtml("A", date, "tempC", row.tempC)}</td>
                <td>${gasInputHtml("A", date, "filterDelta", row.filterDelta)}</td>
                ...
                <td>${gasInputHtml("A", date, "result", row.result)}</td>
