...
const GAS_SECTION_A_DAYS_PER_SHEET = 7;
const GAS_SECTION_B_DAYS_PER_SHEET = 7;
...
function gasJournalMonthDates() {
  const days = new Date(current.year, current.month + 1, 0).getDate();
  const dates = [];
  for (let day = 1; day <= days; day++) dates.push(`${current.year}-${String(current.month + 1).padStart(2, "0")}-${String(day).padStart(2, "0")}`);
  return dates;
}
...
function gasJournalFilledRecord(row, fields) {
...
function gasJournalCompleteB(date = todayISO()) {
  ...
  return ["route", "wells", "gasSmell", "protectionZone", "actions", "checkedBy"].every(field => String(row?.[field] || "").trim());
}
...
function updateGasJournalRow(section, date, field, value) {
  ...
  const next = {
    ...
    checkedBy: old.checkedBy || profile?.name || "",
    updatedAt: now.toISOString(),
    updatedByName: profile?.name || "",
    updatedByRole: profile?.role || "",
    [field]: value
...
function clearGasJournalSheet(section, sheetIndex) {
  const perSheet = section === "A" ? GAS_SECTION_A_DAYS_PER_SHEET : GAS_SECTION_B_DAYS_PER_SHEET;
  const dates = gasJournalMonthDates().slice(sheetIndex * perSheet, sheetIndex * perSheet + perSheet);
  state.gasJournal ||= {};
  dates.forEach(date => delete state.gasJournal[gasJournalKey(section, date)]);
  saveState();
  renderGasJournal();
}
...
function gasInputHtml(section, date, field, value, placeholder = "") {
  return `<input data-gas-section="${section}" data-gas-date="${date}" data-gas-field="${field}" type="text" value="${escapeHtml(value || "")}" placeholder="${escapeHtml(placeholder)}">`;
}

function renderGasJournal() {
  state.gasJournal ||= {};
  ui.aggregateJournalTitle.textContent = "Агрегатный журнал ШГРП / ГРП / ГРУ";
  ui.aggregateJournalMeta.textContent = `${gasJournalFilledCount()} записей. Дата, время и подпись заполняются автоматически. Если сегодня нет записи — кнопка журнала мигает.`;
  const dates = gasJournalMonthDates();
  const sheetsA = [];
  for (let i = 0; i < dates.length; i += GAS_SECTION_A_DAYS_PER_SHEET) sheetsA.push(dates.slice(i, i + GAS_SECTION_A_DAYS_PER_SHEET));
  const sheetsB = [];
  for (let i = 0; i < dates.length; i += GAS_SECTION_B_DAYS_PER_SHEET) sheetsB.push(dates.slice(i, i + GAS_SECTION_B_DAYS_PER_SHEET));
  const sheetA = Number.isInteger(current.gasSheetA) ? current.gasSheetA : 0;
  const sheetB = Number.isInteger(current.gasSheetB) ? current.gasSheetB : 0;
  current.gasSheetA = Math.min(Math.max(sheetA, 0), Math.max(0, sheetsA.length - 1));
  current.gasSheetB = Math.min(Math.max(sheetB, 0), Math.max(0, sheetsB.length - 1));
  const datesA = sheetsA[current.gasSheetA] || [];
  const datesB = sheetsB[current.gasSheetB] || [];
  ui.aggregateJournalList.innerHTML = `
    <div class="aggregate-print-actions gas-journal-actions">
      <button type="button" data-gas-prev-a ${current.gasSheetA <= 0 ? "disabled" : ""}>‹ Раздел А</button>
      <button type="button" data-gas-next-a ${current.gasSheetA >= sheetsA.length - 1 ? "disabled" : ""}>Раздел А ›</button>
      <button type="button" data-gas-print-a>Печать раздела А</button>
      <button type="button" data-gas-clear-a>Удалить лист А №${current.gasSheetA + 1}</button>
      <button type="button" data-gas-prev-b ${current.gasSheetB <= 0 ? "disabled" : ""}>‹ Раздел Б</button>
      <button type="button" data-gas-next-b ${current.gasSheetB >= sheetsB.length - 1 ? "disabled" : ""}>Раздел Б ›</button>
      <button type="button" data-gas-print-b>Печать раздела Б</button>
      <button type="button" data-gas-clear-b>Удалить лист Б №${current.gasSheetB + 1}</button>
    </div>
    <div class="aggregate-journal-sheet gas-journal-sheet" data-print-section="A">
      <div class="aggregate-sheet-head"><strong>Раздел А. Эксплуатация и ТО ГРП (ГРУ)</strong><span>Лист раздела А № ${current.gasSheetA + 1}</span></div>
      <div class="aggregate-journal-table-wrap">
        <table class="aggregate-journal-table gas-journal-table">
          ...
          <tbody>${datesA.map(date => {
            const row = gasJournalRecord("A", date);
            return `<tr class="${date === todayISO() && !gasJournalCompleteA(date) ? "gas-missing-today" : ""}">
              <td>${dateHuman(date)}</td>
              <td>${escapeHtml(row.time || "")}</td>
              <td>${gasInputHtml("A", date, "inletMpa", row.inletMpa, "0,30")}</td>
              <td>${gasInputHtml("A", date, "outletMpa", row.outletMpa, "0,005")}</td>
              <td>${gasInputHtml("A", date, "tempC", row.tempC, "24")}</td>
              <td>${gasInputHtml("A", date, "filterDelta", row.filterDelta, "0,002")}</td>
              <td>${gasSelectHtml("A", date, "regulator", row.regulator, ["Исправно", "Неисправно"])}</td>
              <td>${gasSelectHtml("A", date, "pzk", row.pzk, ["Исправно", "Неисправно"])}</td>
              <td>${gasSelectHtml("A", date, "psk", row.psk, ["Исправно", "Неисправно"])}</td>
              <td>${gasInputHtml("A", date, "result", row.result, "Замечаний нет")}</td>
    ...
    <div class="aggregate-journal-sheet gas-journal-sheet" data-print-section="B">
      <div class="aggregate-sheet-head"><strong>Раздел Б. Обход подземного газопровода</strong><span>Лист раздела Б № ${current.gasSheetB + 1}</span></div>
      <div class="aggregate-journal-table-wrap">
        <table class="aggregate-journal-table gas-journal-table gas-route-table">
          ...
          <tbody>${datesB.map(date => {
            const row = gasJournalRecord("B", date);
            const route = row.route || GAS_ROUTE_LIST.join("\n");
            return `<tr class="${date === todayISO() && !gasJournalCompleteB(date) ? "gas-missing-today" : ""}">
              <td>${dateHuman(date)}</td>
              <td>${escapeHtml(row.time || "")}</td>
              <td class="gas-route-cell">${escapeHtml(route).replace(/\n/g, "<br>")}</td>
              <td>${gasSelectHtml("B", date, "wells", row.wells, ["Исправны", "Неисправны"])}</td>
              ...
              <td>${gasInputHtml("B", date, "remarks", row.remarks, "Нет")}</td>
  ...
  ui.aggregateJournalList.querySelector("[data-gas-prev-a]")?.addEventListener("click", () => { current.gasSheetA--; renderGasJournal(); });
  ui.aggregateJournalList.querySelector("[data-gas-next-a]")?.addEventListener("click", () => { current.gasSheetA++; renderGasJournal(); });
  ui.aggregateJournalList.querySelector("[data-gas-prev-b]")?.addEventListener("click", () => { current.gasSheetB--; renderGasJournal(); });
  ui.aggregateJournalList.querySelector("[data-gas-next-b]")?.addEventListener("click", () => { current.gasSheetB++; renderGasJournal(); });
  ui.aggregateJournalList.querySelector("[data-gas-print-a]")?.addEventListener("click", () => window.print());
  ui.aggregateJournalList.querySelector("[data-gas-print-b]")?.addEventListener("click", () => window.print());
  ui.aggregateJournalList.querySelector("[data-gas-clear-a]")?.addEventListener("click", () => { if (confirm(`Удалить лист раздела А №${current.gasSheetA + 1}?`)) clearGasJournalSheet("A", current.gasSheetA); });
  ui.aggregateJournalList.querySelector("[data-gas-clear-b]")?.addEventListener("click", () => { if (confirm(`Удалить лист раздела Б №${current.gasSheetB + 1}?`)) clearGasJournalSheet("B", current.gasSheetB); });
}
