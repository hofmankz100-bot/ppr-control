...
async function renderAdminMaintenance() {
  ...
  if (tab === "broadcasts") {
    ...
    if (sheet) sheet.innerHTML = `<div class="aggregate-sheet-head"><strong>Объявления сотрудникам</strong><span>Активных: ${broadcasts.filter(item => item.active).length}</span></div><form class="admin-broadcast-form no-print" data-admin-broadcast-form><label><span>Заголовок</span><input name="title" maxlength="200" required placeholder="Например: Плановые работы"></label><label><span>Текст объявления</span><textarea name="text" maxlength="3000" rows="4" required></textarea></label><div class="admin-broadcast-fields"><label><span>Приоритет</span><select name="priority"><option value="normal">Обычный</option><option value="important">Важный</option><option value="critical">Критический</option></select></label><label><span>Показывать до</span><input name="expiresAt" type="datetime-local" value="${tomorrow}" required></label><label><span>Кому показать</span><select name="roles" multiple size="6">${roleEntries.map(([role,item]) => `<option value="${escapeHtml(role)}">${escapeHtml(item.label || role)}</option>`).join("")}</select><small>Ничего не выбирайте — получат все.</small></label></div><button type="submit">Опубликовать объявление</button></form><div class="admin-broadcast-list">${broadcasts.length ? broadcasts.map(item => `<article class="${escapeHtml(item.priority)} ${item.active ? "" : "closed"}"><div><strong>${escapeHtml(item.title)}</strong><span>${escapeHtml(item.text)}</span><small>${escapeHtml(item.author || "Администратор")} · ${escapeHtml(dateTimeHuman(item.createdAt))} · до ${escapeHtml(dateTimeHuman(item.expiresAt))}</small><small>Адресаты: ${item.roles?.length ? item.roles.map(role => escapeHtml(ROLE_ACCESS[role]?.label || role)).join(", ") : "Все сотрудники"} · ознакомились ${Number(item.readCount || 0)} из ${Number(item.recipientCount || 0)}</small></div>${item.active ? `<button type="button" class="secondary no-print" data-close-admin-broadcast="${escapeHtml(item.id)}">Снять</button>` : `<b>Закрыто</b>`}</article>`).join("") : `<div class="empty-state">Объявлений пока нет.</div>`}</div>`;
  }
