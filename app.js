...
const APP_VERSION = "v163-gas-journal-a4-fullsheet";
...
function gasJournalCompleteA(date = todayISO()) {
  ...
  return ["inletMpa", "outletMpa", "tempC", "filterDelta", "regulator", "pzk", "psk", "result", "aRoomState", "aPerformedWorks", "aAdditionalNotes", "checkedBy"].every(field => String(row?.[field] || "").trim());
}

function gasJournalCompleteB(date = todayISO()) {
  const row = gasJournalRecord("B", date);
  return ["wells", "gasSmell", "protectionZone", "remarks", "actions", "bRouteCondition", "bDefects", "bAdditionalMeasures", "checkedBy"].every(field => String(row?.[field] || "").trim());
}
...
function gasTextareaHtml(section, date, field, value) {
  return `<textarea data-gas-section="${section}" data-gas-date="${date}" data-gas-field="${field}" rows="3">${escapeHtml(value || "")}</textarea>`;
}
...
function renderGasJournal() {
  ...
  ui.aggregateJournalList.innerHTML = `
    <div class="aggregate-journal-sheet gas-journal-sheet gas-a4-sheet ${dateA === todayISO() && !completeA ? "gas-missing-today" : ""}" data-print-section="A">
      ...
      <div class="gas-sheet-bottom gas-sheet-bottom-a">
        <div class="gas-field-block"><label>Состояние помещения ГРП / вентиляции</label>${gasTextareaHtml("A", dateA, "aRoomState", rowA.aRoomState)}</div>
        <div class="gas-field-block"><label>Выполненные работы / обслуживание</label>${gasTextareaHtml("A", dateA, "aPerformedWorks", rowA.aPerformedWorks)}</div>
        <div class="gas-field-block gas-field-wide"><label>Дополнительные замечания</label>${gasTextareaHtml("A", dateA, "aAdditionalNotes", rowA.aAdditionalNotes)}</div>
      </div>
    ...
    <div class="aggregate-journal-sheet gas-journal-sheet gas-a4-sheet ${dateB === todayISO() && !completeB ? "gas-missing-today" : ""}" data-print-section="B">
      ...
      <div class="gas-sheet-bottom gas-sheet-bottom-b">
        <div class="gas-field-block"><label>Состояние маршрута / покрытия / знаков</label>${gasTextareaHtml("B", dateB, "bRouteCondition", rowB.bRouteCondition)}</div>
        <div class="gas-field-block"><label>Выявленные дефекты / нарушения</label>${gasTextareaHtml("B", dateB, "bDefects", rowB.bDefects)}</div>
        <div class="gas-field-block gas-field-wide"><label>Дополнительные принятые меры</label>${gasTextareaHtml("B", dateB, "bAdditionalMeasures", rowB.bAdditionalMeasures)}</div>
      </div>
