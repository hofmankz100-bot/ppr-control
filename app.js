...
const APP_VERSION = "v773-turning-shop-qr-routing-1";
...
function ensureWorkerRatingUi() {
  if (!ui.workerRatingButton) {
    ...
    if (quickNav && topbarActions) {
      ...
      if (ui.globalReminderButton) quickNav.append(ui.globalReminderButton);
...
function attendanceRole() {
  return String(authenticatedProfile?.role || profile?.role || "");
}

function attendanceRequired() {
  return ATTENDANCE_WORKER_ROLES.has(attendanceRole());
}

function attendanceAllowsEditing() {
  if (!attendanceRequired()) return true;
  return attendanceStatus?.canEdit === true
    && Date.parse(attendanceStatus?.session?.expiresAt || "") > Date.now();
}
...
function attendanceWhatsAppUrl(phone = "") {
...
async function handleIncomingAttendanceQrFromUrl() {
  ...
  try {
    ...
    window.alert(error.status === 410
      ...
      : (error.message || "Не удалось отметить смену."));
  }
  return true;
}
...
function showAttendanceScanConfirmation({ alreadyActive = false, startedAt = "", expiresAt = "", displayName = "" } = {}) {
  ...
  const overlay = document.createElement("div");
  overlay.className = "attendance-scan-confirmation";
...
async function refreshAttendanceQr() {
  const modal = document.querySelector(".attendance-modal");
...
async function openAttendancePanel() {
  ...
  if (!status?.canMonitor) {
    window.alert("Этот раздел доступен администраторам и инженерам.");
    return;
  }
  ...
  modal.innerHTML = `<div class="attendance-backdrop" data-attendance-close></div>
    <section class="attendance-panel" role="dialog" aria-modal="true" aria-label="Кто на работе">
      <header><div><h2>Кто на работе</h2><p>Отметка открывает рабочему редактирование на 10 часов.</p></div><button type="button" class="attendance-close" data-attendance-close>×</button></header>
      ${status.isAdmin ? `<div class="attendance-admin-grid">
        ...
        <div class="attendance-manual-card">
          ...
          <p>Если камера не сработала, администратор может открыть смену вручную.</p>
          <select data-attendance-worker><option value="">Выберите сотрудника</option>${workerOptions}</select>
  ...
  modal.querySelector("[data-attendance-grant]")?.addEventListener("click", async () => {
    const userKey = modal.querySelector("[data-attendance-worker]")?.value;
    if (!userKey) return window.alert("Выберите сотрудника.");
    await apiJson("/api/attendance/admin", {
      method: "POST",
      body: JSON.stringify({ action: "grant", userKey })
    });
    await refreshAttendancePanel();
  });
  await refreshAttendancePanel();
...
function resumeRealtimeQuietly(awayMs = 0) {
  ...
  const socketConnecting = Boolean(realtimeSocket && realtimeSocket.readyState === WebSocket.CONNECTING);
  const socketStale = awayMs >= 5000 && (socketConnecting || (socketOpen && Date.now() - lastRealtimeMessageAt > 20000));
  if (socketStale) {
  ...
  const eventsConnecting = Boolean(realtimeEventSource && realtimeEventSource.readyState === EventSource.CONNECTING);
  const eventsStale = awayMs >= 5000 && eventsConnecting;
  if (eventsStale) {
    try { realtimeEventSource.close(); } catch {}
    realtimeEventSource = null;
  }
  if (!socketOpen || socketStale || (!eventsOpen && eventsStale)) connectRealtime();
  startRealtimePoll();
...
function canOpenView(view) {
  if (isForkliftDriverProfile()) return view === "equipment";
...
function canEditEquipmentCatalog(equipmentOrId = current.equipmentId) {
  ...
  const individualAccess = activeUserPermission(authenticatedProfile || profile || {}, "equipmentEdit");
  if (!["editor", "engineer", "shop"].includes(role) && !individualAccess) return false;
...
function requestVisibleForRole(req, role) {
  ...
  return role === "all" || role === profile?.role;
}
function requestVisibleForRoleIndicator(req, role) {
  return canSeeRequestRoleIndicator(role) && requestAllowedByUser(req) && role === profile?.role;
}
...
async function confirmClearRecordedData(button) {
  if (!window.confirm("Очистить рабочие записи: комментарии, заявки, простои, журналы и отчёты?")) return;
...
function finalizeJournalPopup(popup, requestedTitle = "") {
  ...
  const printedHeading = String(doc.querySelector("h1, h2, .aggregate-sheet-head strong")?.textContent || "").replace(/\s+/g, " ").trim();
  const baseTitle = requestedTitle || printedHeading || doc.title || "Журнал ППР Контроль";
  const month = typeof selectedJournalMonth === "function" ? selectedJournalMonth() : todayISO().slice(0, 7);
  const title = baseTitle.includes(month) ? baseTitle : `${baseTitle} ${month}`;
...
function shgrpSectionBRouteForQr(equipmentId, nodeIndex) {
  ...
  const grpMatch = source.match(/ГРП\s*[-–—]?\s*Печь\s*№?\s*(1[01]|[1-9])(?!\d)/i)
    || source.match(/Газо\s*регуляторн(?:ый|ого|ому|ым)?\s+пункт(?:\s*\(\s*ГРП\s*\))?\s*№?\s*(1[01]|[1-9])(?!\d)/i);
...
async function handleIncomingNodeQrFromUrl() {
  ...
  if (isTurnerUser() && isTurningShopEquipment(eq)) {
    clearIncomingNodeQrFromUrl();
    current.productionTab = "turning";
    show("welding", false);
    showAppToast("Открыт раздел токарных работ. QR-обход выполняют электромеханики и инженеры.");
    return true;
  }
...
function refreshNodeWalkProgress(eq) {
  ...
  const indexes = ordinaryNodeIndexes(eq);
  const doneCount = indexes.filter(index => isNodeShiftChecked(getRecord(eq.id, index, current.date), shift.key)).length;
  ui.dayStatus.textContent = `${doneCount}/${indexes.length} · ${shift.label}`;
  ui.dayStatus.style.background = doneCount === indexes.length ? "var(--to)" : "var(--nav-soft)";
}
...
function printEquipmentQrCodes(eq) {
  const printableNodes = ordinaryNodeIndexes(eq).map(nodeIndex => ({ nodeIndex, nodeName: eq.nodes[nodeIndex] }));
...
async function scanNodeQrCode(expectedEquipmentId, expectedNodeIndex, statusEl) {
  ...
  return new Promise(resolve => {
    ...
    overlay.querySelector("[data-qr-retry]")?.addEventListener("click", async event => {
      const button = event.currentTarget;
...
function qrWalkProgress(equipmentId, shift = currentWalkShift()) {
  ...
  const indexes = ordinaryNodeIndexes(eq);
  const done = indexes.filter(nodeIndex => isNodeShiftChecked(getRecord(eq.id, nodeIndex, shift.date), shift.key)).length;
  return { done, total: indexes.length };
}
...
function waitingRole(req) {
  return req.stock || req.done ? "done" : "manual";
}
...
function allOpenCommentTargets() {
  const equipmentTargets = Object.entries(state.checks)
    .flatMap(([k, rec]) => {
...
function isTurnerUser(user = profile || authenticatedProfile || {}) {
  if ([user?.role, user?.jobRole, user?.editorPreviewRole].some(value => String(value || "") === "turner")) return true;
  return /(^|\s|[-–—])токарь/i.test([user?.position, user?.jobTitle, user?.profession, ROLE_ACCESS[user?.jobRole]?.label].filter(Boolean).join(" "));
}

function isProductionWorkerProfile(user = profile || {}) {
...
function turningRecords() {
  state.turningJournal ||= {};
  return Object.values(state.turningJournal).filter(item => item && !item.deletedAt);
}

function turningPendingCount() {
  const actor = weldingActor();
...
function bindProductionTabs() {
  ui.weldingPanel?.querySelectorAll("[data-production-tab]").forEach(button => button.addEventListener("click", () => {
    current.productionTab = button.dataset.productionTab;
...
function renderWeldingJournal() {
  current.productionTab ||= isTurnerUser() && !isWelderUser() ? "turning" : "welding";
...
function render() {
  ...
  updateDowntimeBadge();
  updateRoleBadges();
  applyProductionWorkerVisibility();
...
function printFilledJournalDocument(title, tableHtml) {
  ...
  popup.document.write(`<!doctype html><html lang="ru"><head><meta charset="utf-8"><title>${escapeHtml(title)}</title><style>
    ...
    th,td{border:.3mm solid #000;padding:1.2mm;font-size:7pt;line-height:1.25;vertical-align:top;overflow-wrap:anywhere;white-space:normal}
    .gas-print-lines{display:grid;gap:.8mm;text-align:left}.gas-print-line{padding-bottom:.6mm;border-bottom:.15mm solid #aaa}.gas-print-line:last-child{padding-bottom:0;border-bottom:0}
...
function gasSectionBDisplayLines(value, stripShift = false) {
  ...
  return source
    ...
    .map(item => item.trim())
    .filter(Boolean)
    .map(item => stripShift ? item.replace(/^(?:День|Ночь)\s*[·—-]\s*/i, "") : item);
}

function gasSectionBCondensedValue(value, kind) {
  const lines = gasSectionBDisplayLines(value);
  if (kind === "remarks") {
    const actual = lines.filter(line => !/(?:^|[—-]\s*)Замечани(?:й|я)\s+нет\s*$/i.test(line));
    return actual.length ? actual.join("\n") : "Исправно";
  }
  if (kind === "actions") {
    const actual = lines.filter(line => !/(?:^|[—-]\s*)Не\s+требуется\s*$/i.test(line));
    return actual.length ? actual.join("\n") : "Не требуется";
  }
  return String(value || "");
}
...
function gasSectionBTimeLabel(row) {
  const values = [row?.time, row?.route, row?.wells, row?.gasSmell, row?.protectionZone, row?.remarks, row?.actions];
  const shifts = [];
  values.forEach(value => gasSectionBDisplayLines(value).forEach(line => {
    const match = line.match(/^(День|Ночь)\s*[·—-]/i);
    if (match && !shifts.includes(match[1])) shifts.push(match[1]);
  }));
  const time = String(row?.time || "").replace(/^(?:День|Ночь)\s*[·—-]\s*/i, "").trim();
  return [shifts.join(" / "), time].filter(Boolean).join(" — ");
}

function gasSectionBScreenHtml(value, label, stripShift = true) {
  const lines = gasSectionBDisplayLines(value, stripShift);
  if (!lines.length) return `<span class="gas-empty-value" aria-label="${escapeHtml(label)}">—</span>`;
  return `<div class="gas-cell-lines" aria-label="${escapeHtml(label)}">${lines.map(line => `<div class="gas-cell-line">${escapeHtml(line)}</div>`).join("")}</div>`;
}

function gasSectionBValueHtml(value, label, editorHtml, detectionValue = value) {
  const source = String(detectionValue || "").trim();
  const isQrSummary = /\r?\n|;\s*|^(?:День|Ночь)\s*[·—-]/i.test(source);
...
function renderNodes() {
  ...
  ui.equipmentMeta.textContent = `${eq.area} · ${ordinaryNodeIndexes(eq).length} узлов`;
  const alert = hasOpenCommentEquipment(eq.id);
...
function renderSchedule() {
  ...
  ordinaryNodeIndexes(eq).forEach(nodeIndex => {
...
function annualPprMonthWorks(year, row, month) {
  ...
  (state.downtimes || []).filter(item => item?.type !== "production" && Number(item?.equipmentId) === Number(row.eq.id)
    && Number(item?.nodeIndex) === Number(row.nodeIndex) && String(item?.startedAt || item?.startAt || "").startsWith(prefix)).forEach(item => automatic.push({
      id: `downtime:${item.id}`, source: "downtime", type: "АР", date: String(item.startedAt || item.startAt).slice(0, 10),
      description: item.reason || item.comment || "Аварийная поломка", result: item.closeComment || (item.endedAt ? "Устранено" : "Простой активен"),
      performer: item.closedByName || "", status: item.endedAt ? "Выполнено" : "В работе", fixed: Boolean(item.endedAt), linkedCard: item.id
    }));
  const manualIds = new Set(manual.map(item => item.sourceId).filter(Boolean));
...
function annualPprWorkDocumentDialog(year, work, kind, rerender) {
  ...
  if (work.documents?.[kind]?.fixedAt) {
    const documentRecord = work.documents[kind];
  ...
  if (!number) return;
  const comment = window.prompt("Введите основание или краткое содержание акта:", work.documents?.[kind]?.comment || "")?.trim();
...
function openAnnualPprMonthJournal(year, row, month) {
  ...
  const record = annualPprYearRecord(year, true); record.works ||= [];
  const overlay = document.createElement("div"); overlay.className = "annual-ppr-work-overlay";
  const render = () => {
...
function annualPprEquipmentRows(year) {
  ...
  return [...grouped.values()].map(item => {
    const sheetsByMonth = Object.fromEntries(Array.from({ length: 12 }, (_, index) => {
      const month = index + 1;
...
function annualPprSheetWord(count) {
  ...
  const last = value % 10;
  if (value > 10 && value < 20) return "листов";
  if (last === 1) return "лист";
  if (last >= 2 && last <= 4) return "листа";
  return "листов";
}

function annualPprTimesWord(count) {
  const value = Math.abs(Number(count) || 0) % 100;
  const last = value % 10;
...
function annualPprSheetsForEquipmentMonth(year, equipmentId, month) {
  const schedule = pprCalendarMonthData(allEquipment(), year, month - 1).itemsByDate;
  return Object.entries(schedule).flatMap(([date, items]) => {
    const equipmentItems = items.filter(item => Number(item?.equipmentId) === Number(equipmentId));
    if (!equipmentItems.length) return [];
    const sheet = state.pprSheets?.[date] || pprSheetRecord(date);
    const completion = pprSheetCompletion(date);
    const nodes = [...new Set(equipmentItems.map(item => String(item?.node || "").trim()).filter(Boolean))];
    const performers = [...new Set((sheet.rows || []).map(row => String(row?.markedByName || "").trim()).filter(Boolean))];
    return [{ date, sheet, scheduledItems: items, nodes, performers, complete: completion.complete, partial: completion.partial, awaitingApproval: completion.awaitingApproval }];
...
function openAnnualPprEquipmentMonth(year, equipmentId, month) {
  ...
  const render = () => {
    ...
    overlay.innerHTML = `<section class="annual-ppr-work-dialog annual-ppr-node-progress-dialog"><header><div><strong>Листы ППР · ${escapeHtml(equipmentRow.eq.name)}</strong><span>${String(month).padStart(2, "0")}.${year} · выполнено ${progress.done} из ${progress.total}</span></div><button type="button" data-progress-back>← Назад</button></header><div class="annual-ppr-node-progress-list">${progress.sheets.length ? progress.sheets.map(item => {
      const repeated = item.repeatedNodes.length > 0;
      return `<article class="${item.complete ? "done" : "pending"}"><div><strong>${item.complete ? "✓" : item.awaitingApproval ? "!" : "○"} Лист ППР · ${escapeHtml(dateHuman(item.date))}</strong><span>Узлы: ${escapeHtml(item.nodes.join(", ") || "по графику")}</span>${repeated ? `<span class="annual-ppr-repeated-nodes">${item.repeatedNodeCounts.map(entry => `${escapeHtml(entry.node)} — ${escapeHtml(annualPprMonthNames(entry.months))} · ${entry.count} ${annualPprTimesWord(entry.count)} за год`).join("; ")}</span>` : ""}<small>${item.complete ? `Выполнен · принял ${escapeHtml(item.sheet.approvedByName || "инженер")}` : item.awaitingApproval ? "Работы выполнены · ожидает приёмки инженером" : item.partial ? "Заполняется" : "Запланирован"}</small></div><button type="button" data-open-annual-sheet="${escapeHtml(item.date)}">Открыть лист ППР</button></article>`;
    ...
    overlay.querySelectorAll("[data-open-annual-sheet]").forEach(button => button.addEventListener("click", () => {
      ...
      if (!item) return;
      const renderSheet = () => {
        overlay.innerHTML = `<section class="annual-ppr-work-dialog annual-ppr-sheet-dialog"><header><div><strong>Лист ППР · ${escapeHtml(dateHuman(item.date))}</strong><span>${escapeHtml(equipmentRow.eq.name)}</span></div><button type="button" data-sheet-list-back>← К списку</button></header><div class="annual-ppr-sheet-dialog-body">${renderPprMaintenanceSheet(item.date, item.scheduledItems)}</div></section>`;
        overlay.querySelector("[data-sheet-list-back]")?.addEventListener("click", render);
        bindPprCalendarControls(overlay, renderSheet);
      };
      renderSheet();
    }));
  };
...
function annualPprOutputClone(overlay) {
  ...
  const preparedByInput = clone.querySelector('[data-annual-ppr-meta="preparedBy"]');
  ...
  clone.querySelectorAll("input").forEach(input => { const span = document.createElement("span"); span.className = "annual-ppr-print-value"; span.textContent = input.type === "date" && input.value ? dateHuman(input.value) : input.value || ""; input.replaceWith(span); });
...
async function shareAnnualPprPdf(overlay, year, button) {
  try {
    await ensurePprOptionalLibrary("annual-pdf");
  } catch {
    return window.alert("Не удалось загрузить модуль PDF. Проверьте интернет и повторите.");
  }
  const clone = annualPprOutputClone(overlay);
  if (!clone) return;
...
function openAnnualPprSchedule(initialYear = new Date().getFullYear()) {
  ...
  overlay.innerHTML = `<section class="annual-ppr-dialog"><header class="no-print"><div><strong>Годовой график ППР</strong><span>Нажмите месячный счётчик, чтобы открыть листы ППР</span></div><div><label>Год <input data-annual-ppr-year type="number" min="2020" max="2100" value="${year}"></label><button type="button" data-share-annual-ppr-pdf>Скачать / отправить PDF</button><button type="button" data-print-annual-ppr>Печать A4</button><button type="button" data-close-annual-ppr>← Назад</button></div></header><div class="annual-ppr-print-area"><div class="annual-ppr-approval"><div><strong>УТВЕРЖДАЮ</strong><br><strong>Директор завода</strong><br><span class="annual-ppr-sign-caption">Ф.И.О. / подпись</span> <input data-annual-ppr-meta="approvedBy" value="${escapeHtml(record.approvedBy || "")}" placeholder="Ф.И.О. / подпись"><br>«___» __________ ${year} г.</div></div><div class="annual-ppr-print-title"><h1>ГОДОВОЙ ГРАФИК ПЛАНОВО-ПРЕДУПРЕДИТЕЛЬНЫХ РЕМОНТОВ ОБОРУДОВАНИЯ НА ${year} ГОД</h1><div>ТОО «Aluminium of Kazakhstan»</div></div><div class="annual-ppr-meta"><span>Редакция: <input data-annual-ppr-meta="revision" value="${escapeHtml(record.revision || "01")}"></span><span>Сформирован из календаря и листов ППР: ${dateHuman(todayISO())}</span></div><div class="annual-ppr-scroll">${annualPprTableHtml(year)}</div><p class="annual-ppr-note">Формат счётчика: выполнено / должно быть по календарю. Нажмите на счётчик, чтобы открыть список и сам лист ППР.</p><div class="annual-ppr-signatures"><label><strong>Согласовано: директор по производству</strong><span class="annual-ppr-sign-caption">Ф.И.О. / подпись</span><input data-annual-ppr-meta="agreedProductionBy" value="${escapeHtml(record.agreedProductionBy || "")}" placeholder="Ф.И.О. / подпись"></label><label><strong>Согласовано: ответственный за ОТ и ПБ</strong><span class="annual-ppr-sign-caption">Ф.И.О. / подпись</span><input data-annual-ppr-meta="agreedSafetyBy" value="${escapeHtml(record.agreedSafetyBy || "")}" placeholder="Ф.И.О. / подпись"></label><label><strong>Составил: ответственный инженер</strong><span class="annual-ppr-sign-caption">Ф.И.О. / подпись</span><input data-annual-ppr-meta="preparedBy" value="${escapeHtml(record.preparedBy || "")}" placeholder="Ф.И.О. / подпись"></label></div></div></section>`;
  document.body.append(overlay);
...
function hiddenWorkerRatings() {
  const eligibleKeys = new Set(loadUsers()
    .filter(user => isWorkerRatingRole(user.role))
    .map(user => workerRatingKey(user.role, user.name || user.employeeId || user.phone)));
  return (Array.isArray(state.adminConfig?.excludedRatingWorkers) ? state.adminConfig.excludedRatingWorkers : [])
    .filter(item => eligibleKeys.has(String(item?.key || "")));
}
...
async function renderAdminMaintenance() {
  ...
  const integrity = result.integrity && typeof result.integrity === "object" ? result.integrity : { healthy: true, fixableCount: 0, issues: [] };
  const integrityIssues = Array.isArray(integrity.issues) ? integrity.issues : [];
  const integrityCount = integrityIssues.reduce((sum, item) => sum + Number(item.count || 0), 0);
  const size = Number(pg.sizeBytes || 0);
  const sizeText = size ? `${(size / 1024 / 1024).toFixed(1)} МБ` : "нет данных";
  const primaryAdminTabs = new Set(["trash", "backups", "audit", "report"]);
  const technicalAdminTabOpen = !primaryAdminTabs.has(tab);
  ui.adminMaintenancePanel.innerHTML = `
    ...
    <div class="segmented admin-maintenance-tabs no-print">
      ...
      <button type="button" class="${tab === "backups" ? "active" : ""}" data-admin-maintenance-tab="backups">Резервные копии · ${backups.length}</button>
      <button type="button" class="${tab === "audit" ? "active" : ""}" data-admin-maintenance-tab="audit">История действий</button>
      <button type="button" class="${tab === "report" ? "active" : ""}" data-admin-maintenance-tab="report">Состояние данных · ${Number(systemReport.summary?.critical || 0) + Number(systemReport.summary?.warnings || 0)}</button>
      <details class="admin-technical-tools" ${technicalAdminTabOpen ? "open" : ""}>
        <summary>Технические настройки${technicalAdminTabOpen ? " · открыт служебный раздел" : ""}</summary>
        <div class="admin-technical-tools-grid">
  ...
  maintenanceTabs?.querySelector('[data-admin-maintenance-tab="settings"]')?.remove();
  ...
  if (tab === "backups") {
    ...
    if (sheet) sheet.innerHTML = `
      ...
      <div class="admin-backup-actions no-print"><button type="button" data-create-admin-backup>Создать резервную копию</button><a class="button-link secondary" href="/api/export/all" download>Скачать полный экспорт</a></div>
      <div class="empty-state">Перед восстановлением система автоматически создаст страховочную копию. Текущие сеансы сотрудников не заменяются старыми.</div>
      <div class="admin-backup-list">${backups.length ? backups.map(item => `<article><div><strong>${escapeHtml(item.label || "Резервная копия")}</strong><span>${escapeHtml(dateTimeHuman(item.createdAt))} · ${escapeHtml(item.createdBy || "Система")} · ${(Number(item.sizeBytes || 0) / 1024 / 1024).toFixed(1)} МБ</span><small>${escapeHtml(item.storage === "postgres" ? "PostgreSQL" : "Локальная копия")} · контроль ${escapeHtml(String(item.checksum || "нет").slice(0, 12))}</small></div><div class="no-print"><a class="button-link secondary" href="/api/admin/backups/${encodeURIComponent(item.id)}" download>Скачать</a><button type="button" class="danger" data-restore-admin-backup="${escapeHtml(item.id)}">Восстановить</button></div></article>`).join("") : `<div class="empty-state">Резервных копий пока нет.</div>`}</div>`;
  }
  if (tab === "integrity") {
    const sheet = ui.adminMaintenancePanel.querySelector(".admin-maintenance-sheet");
    if (sheet) sheet.innerHTML = `
      <div class="aggregate-sheet-head"><strong>Диагностика целостности данных</strong><span>${dateTimeHuman(integrity.checkedAt || new Date().toISOString())}</span></div>
      <div class="admin-integrity-summary ${integrity.healthy ? "ok" : "warning"}"><strong>${integrity.healthy ? "Нарушений не найдено" : `Найдено записей для проверки: ${integrityCount}`}</strong><span>${Number(integrity.fixableCount || 0)} можно безопасно исправить автоматически. Перед исправлением создаётся резервная копия.</span></div>
      <div class="admin-integrity-list">${integrityIssues.map(item => `<article class="${item.count ? (item.fixable ? "fixable" : "manual") : "ok"}"><div>${item.fixable && item.count ? `<label class="no-print"><input type="checkbox" data-integrity-fix="${escapeHtml(item.id)}"> Выбрать</label>` : ""}<strong>${escapeHtml(item.title || item.id)}</strong><span>${escapeHtml(item.description || "")}</span>${Array.isArray(item.samples) && item.samples.length ? `<small>${item.samples.map(escapeHtml).join("<br>")}</small>` : ""}</div><b>${Number(item.count || 0)}</b></article>`).join("")}</div>
      ${integrity.fixableCount ? `<button type="button" class="no-print" data-fix-integrity>Исправить выбранное безопасно</button>` : ""}
      <div class="empty-state">Повторяющиеся табельные номера, телефоны и неполные карточки не исправляются автоматически. Их нужно проверить в разделе сотрудников.</div>`;
  }
  if (tab === "activity") {
    const sheet = ui.adminMaintenancePanel.querySelector(".admin-maintenance-sheet");
    ...
    if (sheet) sheet.innerHTML = `
      ...
      <div class="admin-activity-list">${activityItems.length ? activityItems.map(item => `<article class="${item.unread ? "unread" : ""}" data-activity-category="${escapeHtml(item.category || "other")}"><time>${escapeHtml(dateTimeHuman(item.at))}</time><div><strong>${escapeHtml(item.actorName || "Система")}</strong><span>${escapeHtml(adminAuditActionLabel(item.action))}</span><small>${escapeHtml([item.targetLabel || item.targetId, item.reason, item.details].filter(Boolean).join(" · "))}</small></div>${item.unread ? `<b title="Новое">●</b>` : ""}</article>`).join("") : `<div class="empty-state">Новых действий сотрудников пока нет.</div>`}</div>`;
  }
  if (tab === "archives") {
    const sheet = ui.adminMaintenancePanel.querySelector(".admin-maintenance-sheet");
    const labels = { audit: "Журнал действий", resolved_alerts: "Закрытые уведомления", restored_trash: "Восстановленные записи корзины", config_history: "Старые версии настроек" };
    if (sheet) sheet.innerHTML = `
      <div class="aggregate-sheet-head"><strong>Архив и сроки хранения</strong><span>Архивов: ${archives.length}</span></div>
      <div class="admin-archive-builder no-print"><label><span>Старше указанного срока</span><select data-archive-days>${[30,90,180,365,730].map(days => `<option value="${days}" ${Number(archivePreview.days) === days ? "selected" : ""}>${days} дней</option>`).join("")}</select></label><div class="admin-archive-categories">${Object.entries(labels).map(([key,label]) => `<label><input type="checkbox" data-archive-category="${key}" ${Number(archivePreview.counts?.[key] || 0) ? "" : "disabled"}><span>${label}</span><b data-archive-count="${key}">${Number(archivePreview.counts?.[key] || 0)}</b></label>`).join("")}</div><button type="button" data-create-admin-archive>Перенести выбранное в архив</button><small>Перед переносом система создаёт полную резервную копию. Архив получает контрольную сумму и остаётся доступным для скачивания.</small></div>
      <div class="admin-archive-list">${archives.length ? archives.map(item => `<article><div><strong>${escapeHtml(item.label || "Архив")}</strong><span>${escapeHtml(dateTimeHuman(item.createdAt))} · ${escapeHtml(item.createdBy || "Администратор")} · ${(Number(item.sizeBytes || 0)/1024).toFixed(1)} КБ</span><small>${escapeHtml(item.storage === "postgres" ? "PostgreSQL" : "Локальное хранилище")} · контроль ${escapeHtml(String(item.checksum || "").slice(0,12))}</small></div><div class="admin-archive-row-actions no-print"><a class="button-link secondary" href="/api/admin/archives/${encodeURIComponent(item.id)}" download>Скачать</a><button type="button" data-restore-admin-archive="${escapeHtml(item.id)}">Восстановить</button></div></article>`).join("") : `<div class="empty-state">Архивов пока нет.</div>`}</div>`;
  }
  if (tab === "automation") {
    const sheet = ui.adminMaintenancePanel.querySelector(".admin-maintenance-sheet");
    if (sheet) sheet.innerHTML = `
      <div class="aggregate-sheet-head"><strong>Автоматическое обслуживание</strong><span>${automation.autoBackupEnabled ? "Включено" : "Отключено"}</span></div>
      <div class="admin-automation-status ${automation.lastError ? "warning" : "ok"}"><article><strong>Последняя успешная копия</strong><b>${automation.lastSuccessAt ? escapeHtml(dateTimeHuman(automation.lastSuccessAt)) : "Ещё не создавалась"}</b></article><article><strong>Следующий запуск</strong><b>${automation.autoBackupEnabled && automation.nextRunAt ? escapeHtml(dateTimeHuman(automation.nextRunAt)) : "Автоматизация отключена"}</b></article><article><strong>Состояние</strong><b>${automation.running ? "Создаётся копия…" : automation.lastError ? escapeHtml(automation.lastError) : "Готово"}</b></article></div>
  ...
  if (tab === "settings") {
    const sheet = ui.adminMaintenancePanel.querySelector(".admin-maintenance-sheet");
    sheet?.insertAdjacentHTML("beforeend", `<section class="admin-qr-route-builder"><div class="aggregate-sheet-head"><strong>QR-маршруты</strong><span>Технические и оперативные обходы</span></div><form data-admin-qr-route-form><label><span>Название</span><input name="name" required></label><label><span>Группа</span><select name="group"><option value="technical">Инженеры и электромеханики</option><option value="operational">Операторы и начальники цехов</option></select></label><label><span>Роль</span><select name="role"><option value="">Все роли группы</option>${Object.entries(ROLE_ACCESS).map(([key,value])=>`<option value="${key}">${escapeHtml(value.label||key)}</option>`).join("")}</select></label><label><span>Цех / участок</span><input name="area"></label><label><span>ID сотрудников через запятую (пусто — все)</span><input name="userIds"></label><label><span>Точки оборудование:узел, по одной в строке</span><textarea name="points" rows="6" placeholder="1:0&#10;1:1&#10;3:2"></textarea></label><button type="submit">Сохранить маршрут</button></form><div data-admin-qr-route-list class="admin-qr-route-list"><div class="empty-state">Загружаем маршруты…</div></div><a class="button-link secondary" href="/api/admin/qr-journal.csv" download>Скачать QR-журнал для Excel</a></section>`);
    apiJson("/api/admin/qr-routes").then(data => { const host=ui.adminMaintenancePanel?.querySelector("[data-admin-qr-route-list]"); if(!host)return; const routes=data.routes||[]; host.innerHTML=routes.length?routes.map(route=>`<article class="${route.active===false?"archived":""}"><div><strong>${escapeHtml(route.name)}</strong><span>${route.group==="operational"?"Операторы и начальники":"Инженеры и электромеханики"} · ${escapeHtml(route.area||"Все участки")}</span><small>Точек: ${(route.points||[]).length} · сотрудников: ${(route.userIds||[]).length||"все"}</small></div>${route.active===false?`<b>Архив</b>`:`<button type="button" class="danger no-print" data-archive-qr-route="${escapeHtml(route.id)}">Архивировать</button>`}</article>`).join(""):`<div class="empty-state">Маршрутов пока нет.</div>`; host.querySelectorAll("[data-archive-qr-route]").forEach(button=>button.addEventListener("click",async()=>{const reason=window.prompt("Причина архивирования маршрута:")?.trim();if(!reason)return;const password=window.prompt("Пароль администратора:");if(!password)return;await apiJson("/api/admin/qr-routes",{method:"POST",body:JSON.stringify({action:"archive",id:button.dataset.archiveQrRoute,reason,password})});renderAdminMaintenance();})); }).catch(()=>{});
  }
  if (tab === "access") {
    const sheet = ui.adminMaintenancePanel.querySelector(".admin-maintenance-sheet");
  ...
  if (tab === "report") {
    const sheet = ui.adminMaintenancePanel.querySelector(".admin-maintenance-sheet");
    const statusLabel = systemReport.status === "ok" ? "Система готова" : systemReport.status === "critical" ? "Требуется внимание" : "Есть рекомендации";
    if (sheet) sheet.innerHTML = `<div class="aggregate-sheet-head"><strong>Контрольный отчёт системы</strong><span>${escapeHtml(dateTimeHuman(systemReport.generatedAt || new Date().toISOString()))}</span></div><div class="admin-report-hero ${escapeHtml(systemReport.status)}"><div><span>Общий статус</span><strong>${statusLabel}</strong></div><div><b>${Number(systemReport.summary?.ok || 0)}</b><span>в норме</span></div><div><b>${Number(systemReport.summary?.warnings || 0)}</b><span>рекомендаций</span></div><div><b>${Number(systemReport.summary?.critical || 0)}</b><span>критических</span></div></div><div class="admin-report-actions no-print"><button type="button" data-refresh-system-report>Обновить проверку</button><a class="button-link secondary" href="/api/admin/system-report?download=1" download>Скачать безопасный отчёт</a></div><div class="admin-report-checks">${(systemReport.checks || []).map(item => `<article class="${escapeHtml(item.status)}"><i>${item.status === "ok" ? "✓" : item.status === "critical" ? "!" : "•"}</i><div><strong>${escapeHtml(item.title)}</strong><span>${escapeHtml(item.detail || "")}</span></div></article>`).join("")}</div><div class="admin-report-metrics"><article><strong>${Number(systemReport.metrics?.users || 0)}</strong><span>сотрудников</span></article><article><strong>${Number(systemReport.metrics?.backups || 0)}</strong><span>полных копий</span></article><article><strong>${(Number(systemReport.metrics?.databaseSizeBytes || 0)/1024/1024).toFixed(1)} МБ</strong><span>база PostgreSQL</span></article><article><strong>${Number(systemReport.metrics?.memoryMb || 0)} МБ</strong><span>память сервера</span></article></div><div class="empty-state">Отчёт не содержит пароли, тексты документов и персональные рабочие записи. Его можно безопасно передать техническому специалисту.</div>`;
  }
  if (tab === "transfer") {
    const sheet = ui.adminMaintenancePanel.querySelector(".admin-maintenance-sheet");
    if (sheet) sheet.innerHTML = `<div class="aggregate-sheet-head"><strong>Перенос административных настроек</strong><span>Без рабочих данных</span></div><div class="admin-transfer-grid"><article><strong>Экспорт</strong><p>Скачать название организации, справочники, пороги мониторинга, расписание копий и тексты инструкций наряда.</p><a class="button-link" href="/api/admin/config-package" download>Скачать пакет настроек</a></article><article><strong>Импорт</strong><p>Выберите ранее скачанный пакет. Сначала система проверит формат и контрольную сумму.</p><label class="button-link secondary"><input type="file" accept="application/json,.json" data-config-package-file hidden>Выбрать JSON-файл</label><div data-config-package-preview class="admin-transfer-preview">Файл ещё не выбран.</div><button type="button" data-import-config-package disabled>Применить проверенный пакет</button></article></div><div class="empty-state">Импорт не удаляет существующие инструкции и права редакторов. Перед применением автоматически создаётся полная резервная копия.</div>`;
  }
  if (tab === "broadcasts") {
    const sheet = ui.adminMaintenancePanel.querySelector(".admin-maintenance-sheet");
    const roleEntries = Object.entries(ROLE_ACCESS);
    const tomorrow = new Date(Date.now() + Number(notificationPolicy.defaultExpiryHours || 24) * 3600000).toISOString().slice(0, 16);
    if (sheet) sheet.innerHTML = `<div class="aggregate-sheet-head"><strong>Объявления и контроль ознакомления</strong><span>Активных: ${broadcasts.filter(item => item.active).length}</span></div><form class="admin-notification-policy no-print" data-admin-notification-policy><strong>Правила по умолчанию</strong><label><span>Приоритет</span><select name="defaultPriority"><option value="normal" ${notificationPolicy.defaultPriority === "normal" ? "selected" : ""}>Обычный</option><option value="important" ${notificationPolicy.defaultPriority === "important" ? "selected" : ""}>Важный</option><option value="critical" ${notificationPolicy.defaultPriority === "critical" ? "selected" : ""}>Критический</option></select></label><label><span>Срок, часов</span><input name="defaultExpiryHours" type="number" min="1" max="720" value="${Number(notificationPolicy.defaultExpiryHours || 24)}"></label><label><span>Напомнить через, часов</span><input name="unreadReminderHours" type="number" min="1" max="168" value="${Number(notificationPolicy.unreadReminderHours || 8)}"></label><button type="submit">Сохранить правила</button></form><form class="admin-broadcast-form no-print" data-admin-broadcast-form><label><span>Заголовок</span><input name="title" maxlength="200" required placeholder="Например: Плановые работы"></label><label><span>Текст объявления</span><textarea name="text" maxlength="3000" rows="4" required></textarea></label><div class="admin-broadcast-fields"><label><span>Приоритет</span><select name="priority"><option value="normal" ${notificationPolicy.defaultPriority === "normal" ? "selected" : ""}>Обычный</option><option value="important" ${notificationPolicy.defaultPriority === "important" ? "selected" : ""}>Важный</option><option value="critical" ${notificationPolicy.defaultPriority === "critical" ? "selected" : ""}>Критический</option></select></label><label><span>Показывать до</span><input name="expiresAt" type="datetime-local" value="${tomorrow}" required></label><label><span>Кому показать</span><select name="roles" multiple size="6">${roleEntries.map(([role,item]) => `<option value="${escapeHtml(role)}">${escapeHtml(item.label || role)}</option>`).join("")}</select><small>Ничего не выбирайте — получат все.</small></label></div><button type="submit">Опубликовать объявление</button></form><div class="admin-broadcast-list">${broadcasts.length ? broadcasts.map(item => `<article class="${escapeHtml(item.priority)} ${item.active ? "" : "closed"}"><div><strong>${escapeHtml(item.title)}</strong><span>${escapeHtml(item.text)}</span><small>${escapeHtml(item.author || "Администратор")} · ${escapeHtml(dateTimeHuman(item.createdAt))} · до ${escapeHtml(dateTimeHuman(item.expiresAt))}</small><small>Адресаты: ${item.roles?.length ? item.roles.map(role => escapeHtml(ROLE_ACCESS[role]?.label || role)).join(", ") : "Все сотрудники"} · ознакомились ${Number(item.readCount || 0)} из ${Number(item.recipientCount || 0)}</small>${item.remindedAt ? `<small>Последнее напоминание: ${escapeHtml(dateTimeHuman(item.remindedAt))}</small>` : ""}<details><summary>Журнал доставки</summary><div class="admin-delivery-list">${(item.recipients || []).map(person => `<span class="${person.readAt ? "read" : "unread"}"><b>${escapeHtml(person.name)}</b><small>${escapeHtml(person.employeeId || ROLE_ACCESS[person.role]?.label || "Сотрудник")} · ${person.readAt ? `прочитано ${escapeHtml(dateTimeHuman(person.readAt))}` : "не ознакомился"}</small></span>`).join("") || "Нет адресатов"}</div></details></div><div class="admin-broadcast-actions no-print">${item.active && Number(item.readCount || 0) < Number(item.recipientCount || 0) ? `<button type="button" data-remind-admin-broadcast="${escapeHtml(item.id)}">Напомнить</button>` : ""}${item.active ? `<button type="button" class="secondary" data-close-admin-broadcast="${escapeHtml(item.id)}">Снять</button>` : `<b>Закрыто</b>`}</div></article>`).join("") : `<div class="empty-state">Объявлений пока нет.</div>`}</div>`;
  }
  ...
  ui.adminMaintenancePanel.querySelector("[data-config-package-file]")?.addEventListener("change", async event => {
    ...
    if (!file) { preview.textContent = "Файл ещё не выбран."; return; }
    if (file.size > 10 * 1024 * 1024) { preview.textContent = "Файл больше 10 МБ и не может быть пакетом настроек."; return; }
    try { const parsed = JSON.parse(await file.text()); const checked = await apiJson("/api/admin/config-package/preview", { method: "POST", timeout: 30000, body: JSON.stringify({ package: parsed }) }); pendingConfigPackage = parsed; const summary = checked.summary || {}; preview.innerHTML = `<strong>Пакет проверен</strong><span>${escapeHtml(summary.companyName || "")}</span><small>Подразделений: ${Number(summary.departments || 0)} · должностей: ${Number(summary.positions || 0)} · инструкций: ${Number(summary.instructions || 0)}</small>`; apply.disabled = false; } catch (error) { preview.textContent = error.message || "Файл повреждён или имеет неверный формат."; }
  });
  ui.adminMaintenancePanel.querySelector("[data-import-config-package]")?.addEventListener("click", async event => {
    if (!pendingConfigPackage) return; const confirm = window.prompt("Для продолжения напишите: ИМПОРТИРОВАТЬ НАСТРОЙКИ"); if (String(confirm || "").trim().toUpperCase() !== "ИМПОРТИРОВАТЬ НАСТРОЙКИ") return; const reason = window.prompt("Укажите причину импорта:")?.trim(); if (!reason) return; const password = window.prompt("Введите пароль администратора:"); if (!password) return;
    await runButtonOperation(event.currentTarget, async () => { await apiJson("/api/admin/config-package/import", { method: "POST", timeout: 180000, body: JSON.stringify({ package: pendingConfigPackage, confirm, reason, password }) }); window.alert("Настройки импортированы. Резервная копия создана."); renderAdminMaintenance(); }, "Создаём копию и импортируем…");
  });
  ui.adminMaintenancePanel.querySelector("[data-admin-broadcast-form]")?.addEventListener("submit", async event => {
    event.preventDefault(); const form = event.currentTarget; const reason = window.prompt("Укажите служебную причину публикации:")?.trim(); if (!reason) return; const password = window.prompt("Введите пароль администратора:"); if (!password) return; const roles = [...form.elements.roles.selectedOptions].map(option => option.value); const submit = form.querySelector('button[type="submit"]');
    await runButtonOperation(submit, async () => { await apiJson("/api/admin/broadcasts", { method: "POST", body: JSON.stringify({ title: form.elements.title.value, text: form.elements.text.value, priority: form.elements.priority.value, expiresAt: new Date(form.elements.expiresAt.value).toISOString(), roles, reason, password }) }); renderAdminMaintenance(); }, "Публикуем…");
  });
  ui.adminMaintenancePanel.querySelector("[data-admin-notification-policy]")?.addEventListener("submit", async event => { event.preventDefault(); const form = event.currentTarget; const reason = window.prompt("Укажите причину изменения правил уведомлений:")?.trim(); if (!reason) return; const password = window.prompt("Введите пароль администратора:"); if (!password) return; await runButtonOperation(form.querySelector('button[type="submit"]'), async () => { await apiJson("/api/admin/notification-policy", { method: "POST", body: JSON.stringify({ defaultPriority: form.elements.defaultPriority.value, defaultExpiryHours: Number(form.elements.defaultExpiryHours.value), unreadReminderHours: Number(form.elements.unreadReminderHours.value), reason, password }) }); renderAdminMaintenance(); }, "Сохраняем…"); });
  ui.adminMaintenancePanel.querySelectorAll("[data-remind-admin-broadcast]").forEach(button => button.addEventListener("click", async () => { const reason = window.prompt("Укажите причину повторного напоминания:")?.trim(); if (!reason) return; const password = window.prompt("Введите пароль администратора:"); if (!password) return; await runButtonOperation(button, async () => { await apiJson("/api/admin/broadcasts/remind", { method: "POST", body: JSON.stringify({ id: button.dataset.remindAdminBroadcast, reason, password }) }); renderAdminMaintenance(); }, "Отправляем…"); }));
  ui.adminMaintenancePanel.querySelectorAll("[data-close-admin-broadcast]").forEach(button => button.addEventListener("click", async () => { const reason = window.prompt("Укажите причину снятия объявления:")?.trim(); if (!reason) return; const password = window.prompt("Введите пароль администратора:"); if (!password) return; await runButtonOperation(button, async () => { await apiJson("/api/admin/broadcasts", { method: "POST", body: JSON.stringify({ action: "close", id: button.dataset.closeAdminBroadcast, reason, password }) }); renderAdminMaintenance(); }, "Снимаем…"); }));
  ui.adminMaintenancePanel.querySelector("[data-archive-days]")?.addEventListener("change", async event => {
    const preview = await apiJson(`/api/admin/archives/preview?days=${encodeURIComponent(event.currentTarget.value)}`, { timeout: 15000 });
    Object.entries(preview.preview?.counts || {}).forEach(([key, count]) => { const badge = ui.adminMaintenancePanel.querySelector(`[data-archive-count="${key}"]`); const input = ui.adminMaintenancePanel.querySelector(`[data-archive-category="${key}"]`); if (badge) badge.textContent = String(count); if (input) { input.disabled = !count; if (!count) input.checked = false; } });
  });
  ui.adminMaintenancePanel.querySelector("[data-create-admin-archive]")?.addEventListener("click", async event => {
    const categories = [...ui.adminMaintenancePanel.querySelectorAll("[data-archive-category]:checked")].map(input => input.dataset.archiveCategory);
    if (!categories.length) { window.alert("Выберите данные для переноса в архив."); return; }
    const confirm = window.prompt("Для продолжения напишите: ПЕРЕНЕСТИ В АРХИВ");
    if (String(confirm || "").trim().toUpperCase() !== "ПЕРЕНЕСТИ В АРХИВ") return;
    const reason = window.prompt("Укажите причину архивирования:")?.trim(); if (!reason) return;
    const password = window.prompt("Введите пароль администратора:"); if (!password) return;
    const days = Number(ui.adminMaintenancePanel.querySelector("[data-archive-days]")?.value || 180);
    await runButtonOperation(event.currentTarget, async () => { const response = await apiJson("/api/admin/archives", { method: "POST", timeout: 180000, body: JSON.stringify({ categories, days, confirm, reason, password }) }); window.alert(`Перенесено в архив: ${Number(response.archivedCount || 0)} записей.`); renderAdminMaintenance(); }, "Создаём копию и архив…");
  });
  ui.adminMaintenancePanel.querySelectorAll("[data-restore-admin-archive]").forEach(button => button.addEventListener("click", async () => {
    const confirm = window.prompt("Для восстановления напишите: ВОССТАНОВИТЬ АРХИВ"); if (String(confirm || "").trim().toUpperCase() !== "ВОССТАНОВИТЬ АРХИВ") return;
    const reason = window.prompt("Укажите причину восстановления:")?.trim(); if (!reason) return;
    const password = window.prompt("Введите пароль администратора:"); if (!password) return;
    await runButtonOperation(button, async () => { const response = await apiJson("/api/admin/archives/restore", { method: "POST", timeout: 180000, body: JSON.stringify({ archiveId: button.dataset.restoreAdminArchive, confirm, reason, password }) }); window.alert(`Восстановлено записей: ${Number(response.restoredCount || 0)}.`); renderAdminMaintenance(); }, "Создаём копию и восстанавливаем…");
  }));
  ...
  ui.adminMaintenancePanel.querySelector("[data-run-auto-backup]")?.addEventListener("click", async event => {
    ...
    const password = window.prompt("Введите пароль администратора:"); if (!password) return;
    await runButtonOperation(event.currentTarget, async () => { await apiJson("/api/admin/automation/run", { method: "POST", timeout: 180000, body: JSON.stringify({ password }) }); window.alert("Резервная копия создана."); renderAdminMaintenance(); }, "Создаём полную копию…");
  });
  ...
  ui.adminMaintenancePanel.querySelectorAll("[data-access-end-sessions]").forEach(button => button.addEventListener("click", async () => { if (!window.confirm("Завершить все активные сеансы этого сотрудника? На всех устройствах потребуется повторный вход.")) return; const reason = window.prompt("Укажите причину завершения сеансов:")?.trim(); if (!reason) return; const password = window.prompt("Введите пароль администратора:"); if (!password) return; await runButtonOperation(button, async () => { const response = await apiJson("/api/admin/user-sessions", { method: "POST", body: JSON.stringify({ userId: button.dataset.accessEndSessions, reason, password }) }); window.alert(`Завершено сеансов: ${Number(response.ended || 0)}.`); renderAdminMaintenance(); }, "Завершаем…"); }));
  ui.adminMaintenancePanel.querySelectorAll("[data-user-permissions-form]").forEach(form => { const save = async (button, payload) => { const reason = window.prompt("Укажите причину изменения прав:")?.trim(); if (!reason) return; const password = window.prompt("Введите пароль администратора:"); if (!password) return; await runButtonOperation(button, async () => { await apiJson("/api/admin/user-permissions", { method: "POST", body: JSON.stringify({ userId: form.dataset.userPermissionsForm, ...payload, reason, password }) }); await loadRemoteUsers(); renderAdminMaintenance(); }, "Сохраняем…"); }; form.addEventListener("submit", event => { event.preventDefault(); const permissions = [...form.querySelectorAll('[name="permissions"]:checked')].map(input => input.value); const value = form.elements.expiresAt.value; save(form.querySelector('button[type="submit"]'), { action: "save", permissions, expiresAt: value ? new Date(value).toISOString() : "" }); }); form.querySelector("[data-reset-user-permissions]")?.addEventListener("click", event => save(event.currentTarget, { action: "reset" })); form.querySelector("[data-copy-user-permissions]")?.addEventListener("click", event => { const sourceName = window.prompt("Введите ФИО или табельный номер сотрудника, от которого скопировать права:")?.trim(); if (!sourceName) return; const source = accessUsers.find(item => [item.name,item.employeeId,item.phone].some(value => String(value || "").toLocaleLowerCase("ru-RU") === sourceName.toLocaleLowerCase("ru-RU"))); if (!source) return window.alert("Сотрудник не найден."); save(event.currentTarget, { action: "copy", sourceUserId: source.id }); }); });
  ui.adminMaintenancePanel.querySelector("[data-admin-activity-filter]")?.addEventListener("change", event => {
    const value = event.currentTarget.value;
    ui.adminMaintenancePanel.querySelectorAll("[data-activity-category]").forEach(row => { row.hidden = value !== "all" && row.dataset.activityCategory !== value; });
  });
  ui.adminMaintenancePanel.querySelector("[data-admin-activity-read]")?.addEventListener("click", async event => {
    await runButtonOperation(event.currentTarget, async () => { await apiJson("/api/admin/activity/read", { method: "POST", body: "{}" }); renderAdminMaintenance(); }, "Сохраняем…");
  });
  ui.adminMaintenancePanel.querySelector("[data-fix-integrity]")?.addEventListener("click", async event => {
    const fixes = [...ui.adminMaintenancePanel.querySelectorAll("[data-integrity-fix]:checked")].map(input => input.dataset.integrityFix);
    if (!fixes.length) { window.alert("Выберите хотя бы один пункт для исправления."); return; }
    const confirm = window.prompt("Перед исправлением будет создана резервная копия. Для продолжения напишите: ИСПРАВИТЬ ДАННЫЕ");
    if (String(confirm || "").trim().toUpperCase() !== "ИСПРАВИТЬ ДАННЫЕ") return;
    const reason = window.prompt("Укажите причину исправления:")?.trim();
    if (!reason) return;
    const password = window.prompt("Введите пароль администратора:");
    if (!password) return;
    await runButtonOperation(event.currentTarget, async () => {
      const response = await apiJson("/api/admin/integrity/fix", { method: "POST", timeout: 180000, body: JSON.stringify({ fixes, confirm, reason, password }) });
      const total = Object.values(response.fixed || {}).reduce((sum, value) => sum + Number(value || 0), 0);
      window.alert(`Исправлено записей: ${total}. Резервная копия создана.`);
      renderAdminMaintenance();
    }, "Создаём копию и исправляем…");
  });
  ...
  ui.adminMaintenancePanel.querySelectorAll("[data-restore-admin-backup]").forEach(button => button.addEventListener("click", async () => {
    ...
    if (String(confirm || "").trim().toUpperCase() !== "ВОССТАНОВИТЬ БАЗУ") return;
    const reason = window.prompt("Укажите причину восстановления:")?.trim();
    if (!reason) return;
    const password = window.prompt("Введите пароль администратора:");
    if (!password) return;
    await runButtonOperation(button, async () => {
  ...
  ui.adminMaintenancePanel.querySelectorAll("[data-resolve-system-alert]").forEach(button => button.addEventListener("click", async () => {
    ...
    await runButtonOperation(button, async () => { await apiJson("/api/admin/monitoring", { method: "POST", body: JSON.stringify({ action: "resolve", alertId: button.dataset.resolveSystemAlert, reason: "Проверено администратором" }) }); renderAdminMaintenance(); }, "Сохраняем…");
  }));
...
function renderAggregateJournal() {
                ...
                <td data-mobile-label="Исполнитель и подтверждение">
                  ...
                  ${repairMode && item.kind === "Замечание" && !item.resolved ? `
                    <span class="no-print">
                      ...
                      <button type="button" class="mini-action" data-admin-repair-legacy-remark="${escapeHtml(item.recordKey)}" data-remark-id="${escapeHtml(item.remarkId)}">Исправить и закрыть</button>
                    </span>
                  ` : ""}
                </td>
              </tr>
  ...
  ui.aggregateJournalList.querySelectorAll("[data-admin-repair-legacy-remark]").forEach(button => {
    button.addEventListener("click", event => runButtonOperation(event.currentTarget, async () => {
      ...
      const remarkId = event.currentTarget.dataset.remarkId || "";
      const repairFields = event.currentTarget.closest(".no-print");
      const performerName = repairFields?.querySelector("[data-repair-performer]")?.value.trim() || "";
      const confirmerName = repairFields?.querySelector("[data-repair-confirmer]")?.value.trim() || "";
      if (!performerName || !confirmerName) {
        showAppToast("Укажите исполнителя и начальника цеха, который подтвердил.", "error");
        return;
      }
      const [equipmentId, nodeIndex, date] = recordKey.split(":");
...
function activeSystemBroadcasts() {
  ...
  return (Array.isArray(state.systemBroadcasts) ? state.systemBroadcasts : [])
    .filter(item => item && item.active !== false
      ...
      && (!Array.isArray(item.roles) || !item.roles.length || item.roles.includes(profile?.role)))
    .slice(-1);
}
