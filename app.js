...
function relatedIssuedRequestForNode(equipmentId, nodeIndex, date, item) {
  // ВАЖНО: замечание нельзя автоматически привязывать к любой выданной заявке
  // по тому же узлу/дате. Иначе обычное "Устранено" по замечанию может
  // ошибочно отправить чужую заявку в бухгалтерию.
  if (!item?.lastRequestId) return null;
  return Object.values(state.requests || {}).find(req =>
    req.id === item.lastRequestId
...
function requestMatchesRole(req, role) {
  ...
  if (role === "accounting") return req.shopInstallApproved && !req.accountingWrittenOff && !req.stock && req.issued && req.mechanicInstalled;
  return false;
}
...
function nodeWalkStatusText(item) {
  if (item.shopInstallApproved && !item.accountingWrittenOff && !item.done && item.issued && item.mechanicInstalled) return "Установка подтверждена. Ждет бухгалтерию";
