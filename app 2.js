...
const APP_VERSION = "v163-compressor-day-row-status-fix";
...
const COMPRESSOR_JOURNAL_META_KEY = "__compressorJournalMeta";

function compressorJournalKey(area, date, compressor) {
  return `${area}::${date}::${compressor}`;
}

function compressorJournalMeta() {
  state.compressorJournal ||= {};
  const today = todayISO();
  const old = state.compressorJournal[COMPRESSOR_JOURNAL_META_KEY] || {};
  if (!old.startDate) {
    state.compressorJournal[COMPRESSOR_JOURNAL_META_KEY] = {
      id: COMPRESSOR_JOURNAL_META_KEY,
      startDate: today,
      updatedAt: new Date().toISOString()
    };
    saveState();
  }
  return state.compressorJournal[COMPRESSOR_JOURNAL_META_KEY];
}

function compressorJournalStartDate() {
  const start = compressorJournalMeta().startDate || todayISO();
  return start > COMPRESSOR_JOURNAL_END_DATE ? COMPRESSOR_JOURNAL_END_DATE : start;
}

function compressorJournalMonthKey(year, month) {
  return `${year}-${String(month + 1).padStart(2, "0")}`;
}

function compressorJournalMinMonthKey() {
  return compressorJournalStartDate().slice(0, 7);
}

function compressorJournalMaxMonthKey() {
  return COMPRESSOR_JOURNAL_END_DATE.slice(0, 7);
}

function compressorJournalClampCurrentMonth() {
  const minKey = compressorJournalMinMonthKey();
  const maxKey = compressorJournalMaxMonthKey();
  let key = compressorJournalMonthKey(current.year, current.month);
  if (key < minKey) {
    current.year = Number(minKey.slice(0, 4));
    current.month = Number(minKey.slice(5, 7)) - 1;
    current.compressorSheetIndex = 0;
  }
  key = compressorJournalMonthKey(current.year, current.month);
  if (key > maxKey) {
    current.year = Number(maxKey.slice(0, 4));
    current.month = Number(maxKey.slice(5, 7)) - 1;
    current.compressorSheetIndex = 0;
  }
}
...
function compressorJournalDateNeedsAttention(area = COMPRESSOR_JOURNAL_AREA, date = todayISO()) {
  return area === COMPRESSOR_JOURNAL_AREA
    && date >= compressorJournalStartDate()
    && date <= todayISO()
    && date <= COMPRESSOR_JOURNAL_END_DATE
    && !compressorJournalDateComplete(area, date);
}

function compressorJournalAnyDueNeedsAttention(area = COMPRESSOR_JOURNAL_AREA) {
  if (area !== COMPRESSOR_JOURNAL_AREA) return false;
  const start = compressorJournalStartDate();
  const end = todayISO() < COMPRESSOR_JOURNAL_END_DATE ? todayISO() : COMPRESSOR_JOURNAL_END_DATE;
  let cursor = new Date(`${start}T00:00:00`);
  const last = new Date(`${end}T00:00:00`);
  while (cursor <= last) {
    const date = cursor.toISOString().slice(0, 10);
    if (compressorJournalDateNeedsAttention(area, date)) return true;
...
function compressorJournalTodayNeedsAttention(area = COMPRESSOR_JOURNAL_AREA) {
  return compressorJournalAnyDueNeedsAttention(area);
}
...
function renderCompressorJournal(area = COMPRESSOR_JOURNAL_AREA) {
  ...
  const currentMonthKey = compressorJournalMonthKey(current.year, current.month);
  const canGoPrevCompressorMonth = currentMonthKey > compressorJournalMinMonthKey();
  const canGoNextCompressorMonth = currentMonthKey < compressorJournalMaxMonthKey();
        ...
        <table class="aggregate-journal-table compressor-journal-table">
          ...
          <tbody>
            ${sheetRows.map((row, index) => {
              const dateComplete = compressorJournalDateComplete(area, row.date);
              const dateMissing = compressorJournalDateNeedsAttention(area, row.date);
              return `
              <tr class="${dateMissing ? "compressor-journal-day-alert" : ""}">
                ${index % COMPRESSOR_JOURNAL_COMPRESSORS.length === 0 ? `<td rowspan="${COMPRESSOR_JOURNAL_COMPRESSORS.length}" class="${dateMissing ? "compressor-journal-alert" : ""}">${dateHuman(row.date)}</td>` : ""}
  ...
  ui.aggregateJournalList.querySelector("[data-compressor-month='prev']")?.addEventListener("click", () => {
    if (!canGoPrevCompressorMonth) return;
    current.month -= 1;
    if (current.month < 0) {
      current.month = 11;
      current.year -= 1;
  ...
  ui.aggregateJournalList.querySelector("[data-compressor-month='next']")?.addEventListener("click", () => {
    if (!canGoNextCompressorMonth) return;
    current.month += 1;
    if (current.month > 11) {
      current.month = 0;
      current.year += 1;
...
function renderEquipment() {
  ...
  equipment
    ...
    .forEach(eq => {
      ...
      const compressorJournalMissingToday = compressorJournalAnyDueNeedsAttention(eq.area);
      ...
      tr.innerHTML = `
        ...
        <td class="equipment-journal-cell area-color-cell"${downtimeStyle}>
          <button type="button" data-aggregate-area="${escapeHtml(eq.area)}" class="${(compressorJournalMissingToday || gasJournalMissingToday) ? "compressor-journal-alert" : ""}">
            ...
            <small>${eq.area === GAS_JOURNAL_AREA ? (gasJournalMissingToday ? "сегодня не заполнено" : "сегодня заполнено") : eq.area === COMPRESSOR_JOURNAL_AREA ? (compressorJournalMissingToday ? "не заполнено" : "выполнено") : `${aggregateJournalCount(eq.area)} записей`}</small>
          </button>
        </td>
      `;
