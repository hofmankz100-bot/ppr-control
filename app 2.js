...
function renderNodeWalkthrough(eq) {
  ...
  eq.nodes.forEach((nodeName, index) => {
    ...
    row.querySelector("[data-node-fixed]").addEventListener("click", event => runButtonOperation(event.currentTarget, () => {
      ...
      if (fixText && canEditComment(item)) appendCommentEntry(item, fixText, item.commentPhoto);
...
function mergeRemoteState(remote = {}) {
  state.checks = mergeChecksLocal(state.checks, remote.checks || {});
...
function blankKind() {
  return { tasks: Array(15).fill(false), walkDone: false, comment: "", commentPhoto: "", commentOwnerRole: "", commentOwnerName: "", commentLog: [], request: "", requestPhoto: "", resolved: false, updatedAt: "" };
}

function touchCheckRecord(rec) {
  if (!rec) return;
  rec.updatedAt = new Date().toISOString();
  if (rec.to) rec.to.updatedAt = rec.updatedAt;
}

function newerCheckRecord(a, b) {
  const ta = Date.parse(a?.updatedAt || a?.to?.updatedAt || "") || 0;
  const tb = Date.parse(b?.updatedAt || b?.to?.updatedAt || "") || 0;
  return tb > ta ? b : a;
}

function mergeChecksLocal(currentChecks = {}, incomingChecks = {}) {
  const out = { ...(currentChecks || {}) };
  Object.entries(incomingChecks || {}).forEach(([k, incoming]) => {
    out[k] = newerCheckRecord(out[k], incoming);
  });
  return out;
}
...
function renderChecklist() {
  ...
  TASKS[current.kind].forEach((task, index) => {
    ...
    input.addEventListener("change", event => {
      if (!canEditChecklist()) return;
      kind.tasks[index] = event.target.checked;
      kind.walkDone = kind.tasks.some(Boolean);
      touchCheckRecord(rec);
      saveState();
      renderChecklist();
    });
    tbody.append(tr);
  ...
  eq.nodes.forEach((nodeName, index) => {
    ...
    row.innerHTML = `
      ...
      <div class="node-walk-field">
        ...
        <div class="node-walk-status">${nodeWalkStatusText(item, eq.id, index, current.date)}</div>
        <div class="node-walk-actions node-comment-actions">
          <button type="button" data-node-submit-comment="${index}" ${canEditThisComment ? "" : "disabled"}>Отправить</button>
          <button type="button" data-node-fixed="${index}" ${fixedButtonDisabled ? "disabled" : ""}>${fixedButtonLabel}</button>
        </div>
    ...
    row.querySelector("input").addEventListener("change", event => {
      ...
      item.tasks[0] = event.target.checked;
      item.walkDone = event.target.checked;
      touchCheckRecord(record(eq.id, index, current.date));
      saveState();
      const nextDone = eq.nodes.filter((_, nodeIndex) => isNodeChecked(record(eq.id, nodeIndex, current.date))).length;
      ui.dayStatus.textContent = `${nextDone}/${eq.nodes.length}`;
      ui.dayStatus.style.background = nextDone === eq.nodes.length ? "var(--to)" : "var(--nav-soft)";
    ...
    row.querySelector("[data-node-submit-comment]").addEventListener("click", async event => {
      const button = event.currentTarget;
      if (button.disabled || !canEditComment(item)) return;
      const text = row.querySelector("[data-node-comment]").value.trim();
      const statusEl = row.querySelector(".node-walk-status");
      if (!text) {
        statusEl.textContent = "Сначала заполните поле";
        return;
      }
      setButtonBusy(button, true, "Выбор...");
      let sendKind = null;
      try {
        sendKind = await askCommentOrRequest();
      } finally {
        if (button.isConnected) setButtonBusy(button, false);
      }
      if (!sendKind) {
        statusEl.textContent = "Отправка отменена. Можно редактировать.";
        return;
      }
      if (button.disabled) return;
      setButtonBusy(button, true, "Отправка...");
      try {
        if (sendKind === "request") {
          const req = createNodeWalkRequestSubmission(eq.id, index, current.date, item, text, item.commentPhoto);
          if (!req) {
            statusEl.textContent = "Заявка не создана: пустой текст";
            return;
          }
          item.commentPhoto = "";
          item.requestStatus = req.status || "shop";
          item.lastRequestId = req.id;
          statusEl.textContent = `Заявка отправлена: ${statusText(req.status || "shop")}`;
        } else {
          appendCommentEntry(item, text, item.commentPhoto);
          item.resolved = false;
          item.mechanicFixed = false;
          statusEl.textContent = "Замечание отправлено";
        }
        saveState();
        await publishStateNow();
        row.querySelector("[data-node-comment]").value = "";
        ...
        statusEl.textContent = "Не отправлено. Проверь интернет и нажми еще раз.";
        console.error(error);
      } finally {
        if (button.isConnected) setButtonBusy(button, false);
      }
    ...
    row.querySelector("[data-node-comment-photo]").addEventListener("change", async event => {
      if (!canEditComment(item)) return;
      const draftText = row.querySelector(`[data-node-comment="${index}"]`)?.value || "";
      beginCommentEdit(item, draftText);
      item.commentPhoto = await readPhotoFile(event.target.files?.[0]);
      setCommentOwner(item);
      item.commentUpdatedAt = new Date().toISOString();
      event.target.value = "";
...
function nodeWalkStatusText(item, equipmentId = current.equipmentId, nodeIndex = current.nodeIndex, date = current.date) {
  const activeReq = item?.lastRequestId ? state.requests?.[item.lastRequestId] : null;
  const anyReq = activeReq || Object.values(state.requests || {}).find(req =>
    Number(req.equipmentId) === Number(equipmentId)
    && Number(req.nodeIndex) === Number(nodeIndex)
    && req.date === date
    && !req.done
  ...
  if (anyReq) return `Заявка отправлена: ${statusText(anyReq.status || "shop")}`;
  if (item.shopInstallApproved && !item.accountingWrittenOff && !item.done) return "Установка подтверждена";
  if (item.mechanicFixed && !item.resolved) return "Устранено. Ждет подтверждения начальника/инженера";
  if (item.resolved) return "Замечание закрыто";
  if (item.request?.trim()) return `Заявка: ${statusText(item.requestStatus || "shop")}`;
  if (hasAnyComment(item) && !item.resolved) return "Есть замечание";
  return "Комментарий / заявка не создана";
}
