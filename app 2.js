...
const APP_VERSION = "v158-fresh-remark-options";
...
function loadState() {
  try {
    ...
    return { checks: {}, requests: {}, inventory: {}, catalog: { equipment: {} }, directorMessages: [], downtimes: [], gasJournal: {} };
  }
}
...
function mergeRemoteState(remote = {}, options = {}) {
  ...
  state.gasJournal = preferRemote ? { ...(remote.gasJournal || {}) } : mergeObjectByFreshnessLocal(state.gasJournal || {}, remote.gasJournal || {});
  localStorage.setItem(STORE_KEY, JSON.stringify(state));
}
...
const GAS_JOURNAL_ROUTES = ["ГРП – Печь №1", "ГРП – Печь №2", "ГРП – Печь №3", "ГРП – Печь №4", "ГРП – Печь №5", "ГРП – Печь №6", "ГРП – Печь №7", "ГРП – Печь №8", "ГРП – Печь №9", "ГРП – Печь №10", "ГРП – Печь №11"];
const GAS_JOURNAL_DAYS_PER_SHEET = 7;

function isGasJournalArea(area = "") {
  const text = String(area || "").toLowerCase();
  if (text.includes("шгрп") || text.includes("грп") || text.includes("гру")) return true;
  return allEquipment().some(eq => eq.area === area && `${eq.name} ${(eq.nodes || []).join(" ")}`.toLowerCase().match(/шгрп|грп|гру/));
}
function gasJournalKey(area, section, date) { return `${area}::${section}::${date}`; }
function gasJournalMonthRows(area, section) {
  state.gasJournal ||= {};
  ...
  for (let day = 1; day <= days; day++) {
    ...
    const id = gasJournalKey(area, section, date);
    rows.push({ id, area, section, date, ...(state.gasJournal[id] || {}) });
  }
  return rows;
}
function gasJournalRowComplete(row, section) {
  const fields = section === "A" ? ["time", "inPressure", "outPressure", "temperature", "filterDelta", "regulator", "pzk", "psk", "result", "signedBy"] : ["time", "controlPipes", "gasSmell", "securityZone", "measures", "signedBy"];
  return fields.every(field => String(row?.[field] || "").trim());
}
function gasJournalTodayNeedsAttention(area) {
  if (!isGasJournalArea(area)) return false;
  const date = todayISO();
  return !gasJournalRowComplete(state.gasJournal?.[gasJournalKey(area, "A", date)] || {}, "A") || !gasJournalRowComplete(state.gasJournal?.[gasJournalKey(area, "B", date)] || {}, "B");
}
function gasJournalFilledCount(area) {
  return Object.values(state.gasJournal || {}).filter(row => row?.area === area).filter(row => ["time","inPressure","outPressure","temperature","filterDelta","regulator","pzk","psk","result","controlPipes","gasSmell","securityZone","remarks","measures","signedBy"].some(field => String(row[field] || "").trim())).length;
}
function updateGasJournalRow(rowId, field, value) {
  state.gasJournal ||= {};
  const [area, section, date] = rowId.split("::");
  const old = state.gasJournal[rowId] || {};
  const now = new Date();
  const next = { ...old, id: rowId, area, section, date, time: old.time || localTimeNow(), signedBy: old.signedBy || profile?.name || "", [field]: value, updatedAt: now.toISOString(), updatedByName: profile?.name || "", updatedByRole: profile?.role || "" };
  state.gasJournal[rowId] = next;
  saveState();
  return next;
}
function deleteGasJournalSheet(area, section, sheetIndex) {
  const rows = gasJournalMonthRows(area, section);
  rows.slice(Number(sheetIndex) * GAS_JOURNAL_DAYS_PER_SHEET, Number(sheetIndex) * GAS_JOURNAL_DAYS_PER_SHEET + GAS_JOURNAL_DAYS_PER_SHEET).forEach(row => delete state.gasJournal[row.id]);
  saveState();
  renderAggregateJournal();
}
function gasChoiceSelect(row, field, values) {
  const currentValue = row[field] || "";
  return `<select data-gas-row="${escapeHtml(row.id)}" data-gas-field="${field}"><option value=""></option>${values.map(value => `<option value="${escapeHtml(value)}" ${currentValue === value ? "selected" : ""}>${escapeHtml(value)}</option>`).join("")}</select>`;
}
function renderGasJournalSectionATable(rows) {
  return `<table class="aggregate-journal-table gas-journal-table"><thead><tr><th>Дата</th><th>Время</th><th>Вход МПа</th><th>Выход МПа</th><th>Темп. °C</th><th>ΔP фильтра</th><th>Регулятор</th><th>ПЗК</th><th>ПСК</th><th>Результаты ТО</th><th>Подпись</th></tr></thead><tbody>${rows.map(row => `<tr class="${gasJournalRowComplete(row, "A") ? "" : "open"}"><td>${dateHuman(row.date)}</td><td data-gas-time="${escapeHtml(row.id)}">${escapeHtml(row.time || "")}</td><td><input data-gas-row="${escapeHtml(row.id)}" data-gas-field="inPressure" value="${escapeHtml(row.inPressure || "")}" inputmode="decimal"></td><td><input data-gas-row="${escapeHtml(row.id)}" data-gas-field="outPressure" value="${escapeHtml(row.outPressure || "")}" inputmode="decimal"></td><td><input data-gas-row="${escapeHtml(row.id)}" data-gas-field="temperature" value="${escapeHtml(row.temperature || "")}" inputmode="decimal"></td><td><input data-gas-row="${escapeHtml(row.id)}" data-gas-field="filterDelta" value="${escapeHtml(row.filterDelta || "")}" inputmode="decimal"></td><td>${gasChoiceSelect(row,"regulator",["Исправно","Неисправно"])}</td><td>${gasChoiceSelect(row,"pzk",["Исправно","Неисправно"])}</td><td>${gasChoiceSelect(row,"psk",["Исправно","Неисправно"])}</td><td><input data-gas-row="${escapeHtml(row.id)}" data-gas-field="result" value="${escapeHtml(row.result || "")}" placeholder="Замечаний нет"></td><td data-gas-sign="${escapeHtml(row.id)}">${escapeHtml(row.signedBy || "")}</td></tr>`).join("")}</tbody></table>`;
}
function renderGasJournalSectionBTable(rows) {
  const routeText = GAS_JOURNAL_ROUTES.join("\n");
  return `<table class="aggregate-journal-table gas-journal-table"><thead><tr><th>Дата</th><th>Время</th><th>Участок</th><th>Контрольные трубки и колодцы</th><th>Запах газа</th><th>Охранная зона</th><th>Замечания</th><th>Принятые меры</th><th>Подпись</th></tr></thead><tbody>${rows.map(row => `<tr class="${gasJournalRowComplete(row, "B") ? "" : "open"}"><td>${dateHuman(row.date)}</td><td data-gas-time="${escapeHtml(row.id)}">${escapeHtml(row.time || "")}</td><td class="gas-route-cell">${escapeHtml(routeText)}</td><td>${gasChoiceSelect(row,"controlPipes",["Исправны","Неисправны"])}</td><td>${gasChoiceSelect(row,"gasSmell",["Нет","Есть"])}</td><td>${gasChoiceSelect(row,"securityZone",["Без нарушений","Нарушение"])}</td><td><input data-gas-row="${escapeHtml(row.id)}" data-gas-field="remarks" value="${escapeHtml(row.remarks || "")}" placeholder="Нет"></td><td>${gasChoiceSelect(row,"measures",["Не требуется","Требуется"])}</td><td data-gas-sign="${escapeHtml(row.id)}">${escapeHtml(row.signedBy || "")}</td></tr>`).join("")}</tbody></table>`;
}
function renderGasJournal(area) {
  current.gasSection ||= "A";
  const section = current.gasSection === "B" ? "B" : "A";
  const rows = gasJournalMonthRows(area, section);
  const sheets = [];
  for (let i = 0; i < rows.length; i += GAS_JOURNAL_DAYS_PER_SHEET) sheets.push(rows.slice(i, i + GAS_JOURNAL_DAYS_PER_SHEET));
  if (!Number.isInteger(current.gasSheetIndex) || current.gasSheetIndex < 0 || current.gasSheetIndex >= sheets.length) current.gasSheetIndex = 0;
  const missingToday = gasJournalTodayNeedsAttention(area);
  const monthName = new Date(current.year, current.month, 1).toLocaleDateString("ru-RU", { month: "long", year: "numeric" });
  ui.aggregateJournalTitle.textContent = `Агрегатный журнал ШГРП / ГРП / ГРУ: ${area}`;
  ui.aggregateJournalMeta.textContent = `${missingToday ? "За сегодня журнал заполнен не полностью" : "За сегодня журнал заполнен"}. Заполненных записей: ${gasJournalFilledCount(area)}.`;
  ui.aggregateJournalList.innerHTML = `<div class="aggregate-print-actions gas-journal-actions"><div class="segmented"><button type="button" data-gas-month="prev">‹</button><strong>${monthName}</strong><button type="button" data-gas-month="next">›</button></div><div class="segmented"><button type="button" class="${section === "A" ? "active" : ""}" data-gas-section="A">Раздел А — ГРП/ГРУ</button><button type="button" class="${section === "B" ? "active" : ""}" data-gas-section="B">Раздел Б — подземный газопровод</button></div></div>${missingToday ? `<div class="empty-state request-alert">Кнопка журнала мигает: за сегодня нет полной записи в разделе А или Б.</div>` : ""}${sheets.map((sheetRows, sheetIndex) => `<div class="aggregate-journal-sheet gas-journal-sheet ${sheetIndex === current.gasSheetIndex ? "" : "compressor-sheet-hidden"}" data-gas-sheet="${sheetIndex}"><div class="aggregate-sheet-head"><strong>${section === "A" ? "Раздел А. Эксплуатация и ТО ГРП (ГРУ)" : "Раздел Б. Обход подземного газопровода"}</strong><span>${monthName} · Лист №${sheetIndex + 1} раздела ${section}</span><div class="gas-sheet-buttons"><button type="button" data-print-gas-sheet="${sheetIndex}">Печатать</button><button type="button" class="danger" data-delete-gas-sheet="${sheetIndex}">Удалить лист</button></div></div><div class="aggregate-journal-table-wrap">${section === "A" ? renderGasJournalSectionATable(sheetRows) : renderGasJournalSectionBTable(sheetRows)}</div></div>`).join("")}`;
  ui.aggregateJournalList.querySelectorAll("[data-gas-section]").forEach(button => button.addEventListener("click", () => { current.gasSection = button.dataset.gasSection; current.gasSheetIndex = 0; renderAggregateJournal(); }));
  ui.aggregateJournalList.querySelector("[data-gas-month='prev']")?.addEventListener("click", () => { current.month -= 1; if (current.month < 0) { current.month = 11; current.year -= 1; } current.gasSheetIndex = 0; renderAggregateJournal(); });
  ui.aggregateJournalList.querySelector("[data-gas-month='next']")?.addEventListener("click", () => { current.month += 1; if (current.month > 11) { current.month = 0; current.year += 1; } current.gasSheetIndex = 0; renderAggregateJournal(); });
  ui.aggregateJournalList.querySelectorAll("[data-print-gas-sheet]").forEach(button => button.addEventListener("click", () => { const sheet = ui.aggregateJournalList.querySelector(`[data-gas-sheet="${CSS.escape(button.dataset.printGasSheet || "0")}"]`); if (!sheet) return; const cleanup = () => { document.body.classList.remove("print-compressor-single"); sheet.classList.remove("print-selected"); window.removeEventListener("afterprint", cleanup); }; document.body.classList.add("print-compressor-single"); sheet.classList.add("print-selected"); window.addEventListener("afterprint", cleanup); window.print(); window.setTimeout(cleanup, 1000); }));
  ui.aggregateJournalList.querySelectorAll("[data-delete-gas-sheet]").forEach(button => button.addEventListener("click", () => { if (confirm("Удалить заполнение этого листа и открыть новый пустой лист?")) deleteGasJournalSheet(area, section, button.dataset.deleteGasSheet); }));
  ui.aggregateJournalList.querySelectorAll("[data-gas-row]").forEach(control => { const commit = event => { const el = event.currentTarget; const saved = updateGasJournalRow(el.dataset.gasRow, el.dataset.gasField, el.value.trim()); const timeCell = ui.aggregateJournalList.querySelector(`[data-gas-time="${CSS.escape(el.dataset.gasRow)}"]`); const signCell = ui.aggregateJournalList.querySelector(`[data-gas-sign="${CSS.escape(el.dataset.gasRow)}"]`); if (timeCell) timeCell.textContent = saved.time || ""; if (signCell) signCell.textContent = saved.signedBy || ""; }; control.addEventListener("change", event => { commit(event); renderAggregateJournal(); }); control.addEventListener("blur", commit); });
}

function aggregateJournalCount(area) {
  if (isGasJournalArea(area)) return gasJournalFilledCount(area);
...
function renderEquipment() {
  ...
  equipment
    ...
    .forEach(eq => {
      ...
      const gasJournalMissingToday = gasJournalTodayNeedsAttention(eq.area);
      tr.innerHTML = `
        ...
        <td class="equipment-journal-cell area-color-cell"${downtimeStyle}>
          <button type="button" data-aggregate-area="${escapeHtml(eq.area)}" class="${gasJournalMissingToday ? "compressor-journal-alert" : ""}">
            <span>Агрегатный журнал</span>
            <strong>${escapeHtml(eq.area)}</strong>
            <small>${isGasJournalArea(eq.area) ? (gasJournalMissingToday ? "сегодня не заполнено" : "сегодня заполнено") : `${aggregateJournalCount(eq.area)} записей`}</small>
          </button>
        </td>
      `;
...
function renderAggregateJournal() {
  ...
  if (isGasJournalArea(selectedArea)) {
    renderGasJournal(selectedArea);
    return;
  }
