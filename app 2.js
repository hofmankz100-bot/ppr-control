...
const APP_VERSION = "v163-compressor-day-due-blink-fix";
        ...
        <table class="aggregate-journal-table compressor-journal-table">
          ...
          <tbody>
            ${sheetRows.map((row, index) => {
              ...
              const dateStatus = dateComplete ? "Заполнено" : (dateMissing ? "Не заполнено" : "");
              return `
              <tr class="${dateMissing ? "compressor-journal-day-alert" : ""}">
                ${index % COMPRESSOR_JOURNAL_COMPRESSORS.length === 0 ? `<td rowspan="${COMPRESSOR_JOURNAL_COMPRESSORS.length}" class="${dateMissing ? "compressor-journal-alert" : ""}">${dateHuman(row.date)}${dateStatus ? `<br><small>${dateStatus}</small>` : ""}</td>` : ""}
      ...
      tr.innerHTML = `
        ...
        <td class="equipment-journal-cell area-color-cell"${downtimeStyle}>
          <button type="button" data-aggregate-area="${escapeHtml(eq.area)}" class="${(compressorJournalMissingToday || gasJournalMissingToday) ? "compressor-journal-alert" : ""}">
            ...
            <small>${eq.area === GAS_JOURNAL_AREA ? (gasJournalMissingToday ? "сегодня не заполнено" : "сегодня заполнено") : eq.area === COMPRESSOR_JOURNAL_AREA ? (compressorJournalMissingToday ? "есть незаполненные дни" : "заполнено") : `${aggregateJournalCount(eq.area)} записей`}</small>
          </button>
        </td>
      `;
