...
test("crane beams are rendered as workshop nodes without a separate home section", () => {
  ...
  assert.match(app, /renderNodeWalkthrough[\s\S]*nestedCraneEquipmentCard/);
  assert.doesNotMatch(app, /Вложенное оборудование/);
  ...
  assert.doesNotMatch(app, /data-crane-shift/);
  assert.doesNotMatch(app, /data-crane-monthly/);
