...
test("SHGRP QR nodes are restored in their historical index order", () => {
  ...
  assert.doesNotMatch(serverSource, /function restorePress2400Catalog\(db\)/);
});
...
test("annual PPR groups nodes by equipment and shows monthly completed counters", () => {
  ...
  assert.doesNotMatch(tableSource, /progress\.planned/);
  assert.match(clientSource, /function openAnnualPprEquipmentMonth[\s\S]*data-open-annual-sheet/);
...
test("annual PPR marks nodes scheduled in multiple months", () => {
  ...
  assert.doesNotMatch(clientSource, /annual-ppr-repeat-badge/);
  assert.doesNotMatch(clientSource, /annual-ppr-repeat-label/);
  assert.doesNotMatch(clientSource, /↻ \$\{entry\.count\}\/год/);
  assert.match(clientSource, /function annualPprTimesWord\(count\)/);
  assert.match(clientSource, /function annualPprMonthNames\(months = \[\]\)/);
  assert.doesNotMatch(clientSource, /Повторяющихся узлов в листе:/);
  ...
  assert.doesNotMatch(clientSource, /Одинаково выделяются все совпавшие появления, включая первое/);
  assert.doesNotMatch(styles, /\.annual-ppr-repeat-badge/);
  assert.doesNotMatch(styles, /article\.repeated/);
});
