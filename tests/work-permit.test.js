...
test("service worker caches the current permit assets", () => {
  assert.match(serviceWorker, /ppr-v487-crane-operator-access/);
  ...
  assert.match(serviceWorker, /styles\.css\?v=482-crane-journals-entry/);
  assert.match(serviceWorker, /app\.js\?v=487-crane-operator-access/);
});
