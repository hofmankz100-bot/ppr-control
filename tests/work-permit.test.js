...
test("work permit is available and loads its mobile PDF dependency", () => {
  ...
  assert.match(html, /modules\/work-permit\.js\?v=467-night-shift-date/);
...
test("service worker caches the current permit assets", () => {
  assert.match(serviceWorker, /ppr-v467-night-shift-date/);
  ...
  assert.match(serviceWorker, /modules\/work-permit\.js\?v=467-night-shift-date/);
  assert.match(serviceWorker, /styles\.css\?v=467-night-shift-date/);
