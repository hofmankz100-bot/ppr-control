...
test("work permit is available and loads its mobile PDF dependency", () => {
  ...
  assert.match(html, /html2pdf\.bundle\.min\.js\?v=v559-clean-journal-output/);
...
test("service worker caches the current permit assets", () => {
  assert.match(serviceWorker, /ppr-v559-clean-journal-output/);
  assert.match(serviceWorker, /html2pdf\.bundle\.min\.js\?v=v559-clean-journal-output/);
  ...
  assert.match(serviceWorker, /styles\.css\?v=v559-clean-journal-output/);
  assert.match(serviceWorker, /app\.js\?v=v559-clean-journal-output/);
  assert.match(serviceWorker, /node_modules\/jsqr\/dist\/jsQR\.js\?v=v559-clean-journal-output/);
});
