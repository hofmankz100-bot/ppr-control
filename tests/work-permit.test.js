...
test("work permit is available and loads its mobile PDF dependency", () => {
  ...
  assert.ok(appVersion && html.includes(`app.min.js?v=${appVersion}`));
  assert.doesNotMatch(html, /html2pdf\.bundle\.min\.js/);
  assert.doesNotMatch(html, /mammoth\.browser\.min\.js/);
...
test("service worker caches the current permit assets", () => {
  ...
  assert.doesNotMatch(serviceWorker, /html2pdf\.bundle\.min\.js/);
  assert.doesNotMatch(serviceWorker, /mammoth\.browser\.min\.js/);
