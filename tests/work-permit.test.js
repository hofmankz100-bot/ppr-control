...
test("work permit is available and loads its mobile PDF dependency", () => {
  ...
  assert.match(html, /html2pdf\.bundle\.min\.js\?v=383-qr-server-status/);
  assert.match(html, /mammoth\.browser\.min\.js\?v=383-qr-server-status/);
  assert.match(html, /modules\/work-permit\.js\?v=383-qr-server-status/);
...
test("service worker caches the current permit assets", () => {
  assert.match(serviceWorker, /ppr-v383-qr-server-status/);
  assert.match(serviceWorker, /html2pdf\.bundle\.min\.js\?v=383-qr-server-status/);
  assert.match(serviceWorker, /mammoth\.browser\.min\.js\?v=383-qr-server-status/);
  assert.match(serviceWorker, /modules\/work-permit\.js\?v=383-qr-server-status/);
  assert.match(serviceWorker, /styles\.css\?v=383-qr-server-status/);
