...
test("work permit is available and loads its mobile PDF dependency", () => {
  ...
  assert.match(html, /modules\/work-permit\.js\?v=465-aggregate-corrections/);
...
test("service worker caches the current permit assets", () => {
  assert.match(serviceWorker, /ppr-v465-aggregate-corrections/);
  ...
  assert.match(serviceWorker, /modules\/work-permit\.js\?v=465-aggregate-corrections/);
  assert.match(serviceWorker, /styles\.css\?v=465-aggregate-corrections/);
