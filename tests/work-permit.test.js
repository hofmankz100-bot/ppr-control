...
test("work permit is available and loads its mobile PDF dependency", () => {
  ...
  assert.match(html, /html2pdf\.bundle\.min\.js\?v=v598-role-scoped-node-access-1/);
...
test("service worker caches the current permit assets", () => {
  assert.match(serviceWorker, /ppr-v598-role-scoped-node-access-1/);
  assert.match(serviceWorker, /html2pdf\.bundle\.min\.js\?v=v598-role-scoped-node-access-1/);
  ...
  assert.match(serviceWorker, /styles\.css\?v=v598-role-scoped-node-access-1/);
  assert.match(serviceWorker, /app\.js\?v=v598-role-scoped-node-access-1/);
  assert.match(serviceWorker, /node_modules\/jsqr\/dist\/jsQR\.js\?v=v598-role-scoped-node-access-1/);
});
