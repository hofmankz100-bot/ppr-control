...
test("work permit is available and loads its mobile PDF dependency", () => {
  ...
  assert.match(html, /modules\/work-permit\.js\?v=468-admin-data-fast-load/);
...
test("service worker caches the current permit assets", () => {
  assert.match(serviceWorker, /ppr-v468-admin-data-fast-load/);
  ...
  assert.match(serviceWorker, /modules\/work-permit\.js\?v=468-admin-data-fast-load/);
  assert.match(serviceWorker, /styles\.css\?v=468-admin-data-fast-load/);
