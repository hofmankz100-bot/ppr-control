...
test("service worker caches the current permit assets", () => {
  assert.match(serviceWorker, /ppr-v526-production-request-dedup/);
  ...
  assert.match(serviceWorker, /styles\.css\?v=526-production-request-dedup/);
  assert.match(serviceWorker, /app\.js\?v=526-production-request-dedup/);
  assert.match(serviceWorker, /node_modules\/jsqr\/dist\/jsQR\.js\?v=v526-production-request-dedup/);
});
