...
test("work permit is available and loads its mobile PDF dependency", () => {
  ...
  assert.match(html, /modules\/work-permit\.js\?v=473-live-employee-areas/);
...
test("service worker caches the current permit assets", () => {
  assert.match(serviceWorker, /ppr-v473-live-employee-areas/);
  ...
  assert.match(serviceWorker, /modules\/work-permit\.js\?v=473-live-employee-areas/);
  assert.match(serviceWorker, /styles\.css\?v=470-order-visibility/);
  assert.match(serviceWorker, /app\.js\?v=473-live-employee-areas/);
});
