...
test("work permit is available and loads its mobile PDF dependency", () => {
  ...
  assert.match(html, /html2pdf\.bundle\.min\.js\?v=v567-crane-shift-qr-outside-index/);
...
test("service worker caches the current permit assets", () => {
  assert.match(serviceWorker, /ppr-v567-crane-shift-qr-outside-index/);
  assert.match(serviceWorker, /html2pdf\.bundle\.min\.js\?v=v567-crane-shift-qr-outside-index/);
  ...
  assert.match(serviceWorker, /styles\.css\?v=v567-crane-shift-qr-outside-index/);
  assert.match(serviceWorker, /app\.js\?v=v567-crane-shift-qr-outside-index/);
  assert.match(serviceWorker, /node_modules\/jsqr\/dist\/jsQR\.js\?v=v567-crane-shift-qr-outside-index/);
});
