...
const printAssets = fs.readFileSync(path.join(root, "modules", "print-assets.js"), "utf8");
...
test("work permit is available and loads its mobile PDF dependency", () => {
  ...
  assert.match(printAssets, /if \(name === "mammoth"\) return loadOptionalScript\("\/node_modules\/mammoth\/mammoth\.browser\.min\.js"/);
  assert.match(printAssets, /if \(name === "html2pdf"\) return loadOptionalScript\("\/node_modules\/html2pdf\.js\/dist\/html2pdf\.bundle\.min\.js"/);
  assert.match(printAssets, /if \(name === "annual-pdf"\)[\s\S]*?html2canvas\.min\.js[\s\S]*?jspdf\.umd\.min\.js/);
  assert.doesNotMatch(html, /modules\/work-permit\.js/);
  assert.match(printAssets, /if \(name === "work-permit"\) return loadOptionalScript\("\/modules\/work-permit\.js"/);
  assert.match(app, /window\.PprPrintAssets\.createOptionalLibraryLoader\(APP_VERSION\)/);
...
test("permit draft and print-safe values are preserved", () => {
  ...
  assert.match(permit, /@page\s*\{[\s\S]*?size:\s*A4 portrait;[\s\S]*?margin:\s*5mm 6mm/);
});
...
test("official print uses only the selected language and omits instruction bodies", () => {
  ...
  assert.match(permit, /margin:\s*\[5, 6, 5, 6\]/);
...
test("phone layout is card based with final actions and PDF sharing", () => {
  ...
  assert.match(permit, /html2canvas:[\s\S]*?windowWidth:\s*1200[\s\S]*?windowHeight:\s*1120/);
...
test("permit completion fields are optional and official output is compact A4", () => {
  assert.match(permit, /\^\(finish_date\|finish_time\)\$/);
  assert.match(permit, /margin:\s*5mm 6mm/);
  assert.match(permit, /grid-template-columns:\s*repeat\(2,\s*minmax\(0,\s*1fr\)\)/);
  assert.match(permit, /work-permit-safety-list[\s\S]*?grid-template-columns:\s*repeat\(2/);
  assert.match(permit, /work-permit-completion-checks[\s\S]*?font-size:\s*6pt/);
});
...
test("service worker caches the current permit assets", () => {
  ...
  assert.ok(serviceWorker.includes(`modules/print-assets.js?v=${appVersion}`));
