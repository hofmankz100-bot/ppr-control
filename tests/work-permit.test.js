...
test("service worker caches the current permit assets", () => {
  assert.match(serviceWorker, /ppr-v513-live-qr-scanner-only/);
  ...
  assert.match(serviceWorker, /styles\.css\?v=513-live-qr-scanner-only/);
  assert.match(serviceWorker, /app\.js\?v=513-live-qr-scanner-only/);
  assert.match(serviceWorker, /node_modules\/jsqr\/dist\/jsQR\.js\?v=v513-live-qr-scanner-only/);
});
