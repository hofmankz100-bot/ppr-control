...
test("work permit is available and loads its mobile PDF dependency", () => {
  ...
  assert.match(html, /html2pdf\.bundle\.min\.js\?v=v534-technical-rating-only/);
...
test("service worker caches the current permit assets", () => {
  assert.match(serviceWorker, /ppr-v534-technical-rating-only/);
  assert.match(serviceWorker, /html2pdf\.bundle\.min\.js\?v=v534-technical-rating-only/);
  ...
  assert.match(serviceWorker, /styles\.css\?v=v534-technical-rating-only/);
  assert.match(serviceWorker, /app\.js\?v=v534-technical-rating-only/);
  assert.match(serviceWorker, /node_modules\/jsqr\/dist\/jsQR\.js\?v=v534-technical-rating-only/);
});
