...
test("work permit is available and loads its mobile PDF dependency", () => {
  ...
  assert.match(html, /html2pdf\.bundle\.min\.js\?v=v538-ordered-node-actions/);
...
test("service worker caches the current permit assets", () => {
  assert.match(serviceWorker, /ppr-v538-ordered-node-actions/);
  assert.match(serviceWorker, /html2pdf\.bundle\.min\.js\?v=v538-ordered-node-actions/);
  ...
  assert.match(serviceWorker, /styles\.css\?v=v538-ordered-node-actions/);
  assert.match(serviceWorker, /app\.js\?v=v538-ordered-node-actions/);
  assert.match(serviceWorker, /node_modules\/jsqr\/dist\/jsQR\.js\?v=v538-ordered-node-actions/);
});
