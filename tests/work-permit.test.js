...
test("work permit is available and loads its mobile PDF dependency", () => {
  ...
  assert.match(html, /html2pdf\.bundle\.min\.js\?v=v579-dark-admin-records/);
...
test("service worker caches the current permit assets", () => {
  assert.match(serviceWorker, /ppr-v579-dark-admin-records/);
  assert.match(serviceWorker, /html2pdf\.bundle\.min\.js\?v=v579-dark-admin-records/);
  ...
  assert.match(serviceWorker, /styles\.css\?v=v579-dark-admin-records/);
  assert.match(serviceWorker, /app\.js\?v=v579-dark-admin-records/);
  assert.match(serviceWorker, /node_modules\/jsqr\/dist\/jsQR\.js\?v=v579-dark-admin-records/);
});
