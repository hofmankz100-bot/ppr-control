...
test("work permit is available and loads its mobile PDF dependency", () => {
  ...
  assert.match(html, /modules\/work-permit\.js\?v=462-resolution-session-fix/);
...
test("service worker caches the current permit assets", () => {
  assert.match(serviceWorker, /ppr-v462-resolution-session-fix/);
  ...
  assert.match(serviceWorker, /modules\/work-permit\.js\?v=462-resolution-session-fix/);
  assert.match(serviceWorker, /styles\.css\?v=462-resolution-session-fix/);
