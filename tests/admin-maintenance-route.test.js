...
function createHarness(database = { users: [], adminTrash: [] }) {
  const responses = [];
  const events = [];
...
test("admin maintenance restores equipment and its linked GPM records", async () => {
  const database = {
    ...
    gpmJournal: { equipment: {}, inspections: {}, events: {}, managers: {} },
    adminTrash: [{
      id: "trash-1",
      type: "equipment",
      targetId: "12",
      label: "Станок",
      snapshot: { catalogItem: { name: "Станок" }, gpmItems: [{ id: "gpm-1", deleted: true }] }
    }]
  };
  const { handler, responses, events } = createHarness(database);
...
test("admin maintenance backs up and permanently removes deleted equipment", async () => {
  const database = {
    users: [],
    catalog: { equipment: { 12: { id: 12, deleted: true } } },
    gpmJournal: { equipment: { linked: { id: "linked", sourceEquipmentId: 12, deleted: true } } },
